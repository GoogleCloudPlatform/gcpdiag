...
def run_connectivity_test(
  project_id: str, src_ip: str, dest_ip: str, dest_port: int, protocol: str
):
  ...
  # test input
  test_input = {
    'source': {'ipAddress': src_ip, 'networkType': 'GCP_NETWORK'},
    'destination': {'ipAddress': dest_ip, 'port': dest_port},
    'protocol': protocol,
  }

  create_request = (
    networkmanagement.projects()
    .locations()
    .global_()
    .connectivityTests()
    .create(parent=f'projects/{project_id}/locations/global', testId=test_id, body=test_input)
  ).execute()
  ...
  if create_status['done']:
    ...
    res = (
      ...
      .global_()
      .connectivityTests()
      .get(name=f'projects/{project_id}/locations/global/connectivityTests/{test_id}')
    )
    result = res.execute()
