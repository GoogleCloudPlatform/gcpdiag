...
ENTRIES2 = [
  {'one': {'two': 'five', 'data': 'alpha'}},
  {'one': {'two': 'eight', 'data': 'beta'}},
  {'one': {'two': 'eight', 'data': 'gamma'}},
  {'one': {'two': 'six'}},
  {'one': {'two': 'eight'}},
  {'one': {'two': 'eight', 'data': 'gamma'}},
]
...
class TestLogsQuery(TestCase):
  ...
  def setUp(self):
    self._logs_query_fn = FakeLogsQueryFn(
      entries={
        ('mytestproject', 'mytestresource', 'mytestlogname', 'one.two="three"'): FakeLogsResult(
          ENTRIES1
        ),
        ('mytestproject', 'mytestresource', 'mytestlogname', 'one.two="seven"'): FakeLogsResult(
          ENTRIES1
        ),
        ('mytestproject', 'mytestresource', 'mytestlogname', 'one.two="eight"'): FakeLogsResult(
          ENTRIES2
        ),
        ('emptylogs', 'mytestresource', 'mytestlogname', 'one.two="three"'): FakeLogsResult([]),
        ('noresult', 'mytestresource', 'mytestlogname', 'one.two="three"'): FakeLogsResult([]),
      }
    )

  def _create_query(self, search_exprs, project_id=None):
    return LogsQuery(
      project_id=project_id or 'mytestproject',
  ...
  def test_no_result(self):
    query = self._create_query([Equals(field='one.two', value='three')], project_id='noresult')
    query.mk_query()
    self.assertFalse(query.has_matching_entries)

  def test_empty_entries(self):
    query = self._create_query([Equals(field='one.two', value='three')], project_id='emptylogs')
    query.mk_query()
    self.assertFalse(query.has_matching_entries)
  ...
  def test_unique(self):
    ...
    query.mk_query()
    unique = query.get_unique(lambda e: get_path(e, ('one', 'data'), default='unknown'))
