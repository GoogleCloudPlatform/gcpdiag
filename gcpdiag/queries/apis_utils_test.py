...
class RequestMock(apis_stub.ApiStub):
  ...
  def __init__(
    self, n: int, fail_count: int = 0, fail_status: int = 429, uri: str = 'googleapis.com'
  ):
    self.n = n
    if fail_count:
...
class Test(unittest.TestCase):
  ...
  def test_multi_list_all(self):
    results = list(
      apis_utils.multi_list_all(
        requests=[RequestMock(1), RequestMock(3)], next_function=next_function_mock
      )
    )
  ...
  def test_batch_execute_all(self):
    api = apis_stub.get_api_stub('compute', 'v1')
    results = list(apis_utils.batch_execute_all(api, [RequestMock(1), RequestMock(3)]))
    # requests
  ...
  def test_batch_execute_all_unretriable_exception(self):
    ...
    results = list(
      apis_utils.batch_execute_all(
        api, [RequestMock(1, fail_count=1, fail_status=403), RequestMock(3)]
      )
    )
    assert isinstance(results[0][2], utils.GcpApiError) and results[0][2].status == 403

  def test_batch_execute_all_too_many_failures(self):
    api = apis_stub.get_api_stub('compute', 'v1')
    results = list(
      apis_utils.batch_execute_all(
        api, [RequestMock(1, fail_count=config.API_RETRIES + 1, fail_status=429), RequestMock(3)]
      )
    )
  ...
  def test_batch_execute_all_retriable_exception(self):
    ...
    results = list(
      apis_utils.batch_execute_all(
        api, [RequestMock(1, fail_count=config.API_RETRIES, fail_status=429), RequestMock(3)]
      )
    )
  ...
  def test_execute_concurrently_api_server(self, mock_get_executor, mock_execute_single_request):
    api = apis_stub.get_api_stub('compute', 'v1')
  ...
  def test_execute_concurrently_cli(self, mock_batch_execute_all):
    ...
    mock_batch_execute_all.return_value = iter(
      [
        (RequestMock(1), {'items': ['a', 'b']}, None),
        (RequestMock(3), {'items': ['e']}, None),
      ]
    )
...
class ExecuteConcurrentlyWithPaginationTest(unittest.TestCase):
  ...
  def test_cli_context_uses_batch_list_all(self, mock_batch_list_all):
    ...
    mock_batch_list_all.assert_called_once_with(
      self.mock_api, requests, next_function_mock, 'testing', 'items'
    )
  ...
  def test_api_context_single_page(self, mock_execute_concurrently):
    ...
    mock_execute_concurrently.return_value = iter(
      [
        (self.request_1, {'items': ['item1']}, None),
      ]
    )

    results = list(
      apis_utils.execute_concurrently_with_pagination(
        self.mock_api, requests, mock.Mock(), context, log_text='testing', response_keyword='items'
      )
    )
  ...
  def test_api_context_multi_page(self, mock_execute_concurrently):
    ...
    context = models.Context(project_id='test-project', context_provider='api')
    req1, resp1 = mock.Mock(uri='uri1'), {'items': ['item1'], 'nextPageToken': 'page2'}
    ...
    def next_func(previous_request, previous_response):
    ...
    results = list(
      apis_utils.execute_concurrently_with_pagination(
        self.mock_api, [req1], next_func, context, log_text='testing', response_keyword='items'
      )
    )
  ...
  def test_api_context_skips_404(self, mock_execute_concurrently):
    ...
    context = models.Context(project_id='test-project', context_provider='api')
    requests = [self.request_1]
    http_error_404 = googleapiclient.errors.HttpError(
      resp=httplib2.Response({'status': 404}), content=b'Not Found'
    )
    mock_execute_concurrently.return_value = iter(
      [
  ...
  def test_api_context_raises_other_error(self, mock_execute_concurrently):
    ...
    context = models.Context(project_id='test-project', context_provider='api')
    requests = [self.request_1]
    http_error_500 = googleapiclient.errors.HttpError(
      resp=httplib2.Response({'status': 500}), content=b'Server Error'
    )
    mock_execute_concurrently.return_value = iter(
      [
