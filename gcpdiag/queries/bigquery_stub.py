...
class GetJobRequest(apis_stub.ApiStub):
  ...
  def __init__(self, project_id: str, location: str, job_id: str):
    ...
    self.short_job_id = job_id.split('.')[-1].split('_')[-1]  # More robust split
  ...
  def execute(self, num_retries=0):
    ...
    try:
      ...
      logging.warning('Stub: File not found for GetJobRequest: %s. Raising 404.', file_path)
...
class GetQueryResultsRequest(apis_stub.ApiStub):
  ...
  def __init__(self, project_id: str, location: str, job_id: str):
    ...
    logging.debug('Stub: GetQueryResultsRequest prepared for %s', self.json_basename)
  ...
  def execute(self, num_retries=0):
    ...
    try:
      ...
      logging.warning(
        'Stub: File not found for GetQueryResultsRequest: %s. Returning empty success.',
        file_path,
      )
      return {
...
class BigQueryJobsStub(apis_stub.ApiStub):
  ...
  def insert(self, projectId: str, body: dict[str, Any]):
    ...
    job_id = job_ref.get('jobId', f'gcpdiag_query_{uuid.uuid4()}')  # Use provided or generate
    ...
    insert_response = {
      ...
      'status': {'state': 'PENDING'},  # Initial state reported by insert
  ...
  def get(self, projectId: str, jobId: str, location: Optional[str] = None):
    ...
    if jobId in BigQueryJobsStub._job_states and BigQueryJobsStub._job_states[jobId]:
      next_state = BigQueryJobsStub._job_states[jobId].pop(0)  # Get and remove the next state
      logging.debug(
      ...
      next_state.setdefault('status', {}).setdefault('state', 'UNKNOWN')  # Default state if missing
      ...
      # file or 404
      logging.warning(
        'Stub: No predefined state for job %s. Falling back to file-based GetJobRequest.',
        jobId,
      )
      return GetJobRequest(project_id=projectId, location=effective_location, job_id=jobId)

  def getQueryResults(self, projectId: str, jobId: str, location: Optional[str] = None, **kwargs):
    """Mocks jobs.getQueryResults(). Returns a custom request object."""
    logging.debug('Stub: jobs.getQueryResults called for %s, location %s', jobId, location)
    effective_location = location or 'us'  # Use default if None
    return GetQueryResultsRequest(project_id=projectId, location=effective_location, job_id=jobId)
  ...
  def query(self, projectId: str, body: dict[str, Any]):
    ...
    if job_id_match and schema_match:
      ...
      if 'INFORMATION_SCHEMA' in query_text.upper():
        logging.warning(
          'Stub: jobs.query could not parse job/region from INFO_SCHEMA query: %s',
          query_text,
        )
      else:
        # This could be a direct query call, not handled by get_query_results
        # flow
        logging.error('Stub: jobs.query received unexpected query type: %s', query_text)
        # Return an error or specific response for direct queries if needed
        raise NotImplementedError("Stub doesn't handle direct jobs.query calls yet.")
...
class BigQueryApiStub(apis_stub.ApiStub):
  ...
  def new_batch_http_request(self, callback=None):
    raise NotImplementedError('Batch requests not implemented in BigQueryApiStub yet')
