...
class TestBilling:
  ...
  def test_get_billing_account(self):
    ...
    assert (len(billing_account.list_projects(context)) > 0) == DUMMY_BILLING_ACCOUNT_HAS_PROJECTS
  ...
  def test_get_all_projects_in_billing_account(self):
    context = models.Context(project_id=DUMMY_PROJECT_ID)
    billing_account = billing.get_billing_account(context.project_id)
    projects = billing.get_all_projects_in_billing_account(context, billing_account.name)
  ...
  def test_get_billing_info(self):
    ...
    assert project_billing_info.is_billing_enabled() == DUMMY_PROJECT_BILLING_ENABLED

  def test_get_cost_insights_for_a_project(self):
    context = models.Context(project_id=DUMMY_PROJECT_ID)
    cost_insights = billing.get_cost_insights_for_a_project(context.project_id)[0]
