...
DUMMY_INSTANCE2_PATH = (
  f'projects/{DUMMY_PROJECT_NAME}/zones/{DUMMY_ZONE}/instances/{DUMMY_INSTANCE2_NAME}'
)
DUMMY_INSTANCE2_LABELS = {'gcpdiag-test': 'mig'}
DUMMY_INSTANCE3_NAME = 'gke-gke1-default-pool-35923fbc-2xxp'
DUMMY_INSTANCE3_PATH = (
  f'projects/{DUMMY_PROJECT_NAME}/zones/{DUMMY_ZONE}/instances/{DUMMY_INSTANCE3_NAME}'
)
...
DUMMY_NEG1_PATH = (
  f'projects/{DUMMY_PROJECT_NAME}/zones/{DUMMY_ZONE3}/networkEndpointGroups/{DUMMY_NEG1_NAME}'
)
...
class TestGce(unittest.TestCase):
  ...
  def test_get_instances(self):
    ...
    assert instances_by_path[DUMMY_INSTANCE1_PATH].full_path == DUMMY_INSTANCE1_PATH
    assert (
      instances_by_path[DUMMY_INSTANCE1_PATH].short_path
  ...
  def test_get_instances_by_region_returns_instance(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, locations=['fake-region', DUMMY_REGION])
    instances = gce.get_instances(context)
    instances_by_path = {i.full_path: i for i in instances.values()}
    assert DUMMY_INSTANCE1_PATH in instances_by_path and len(instances) == 6

  def test_get_instances_by_label(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, labels=DUMMY_INSTANCE1_LABELS)
    instances = gce.get_instances(context)
    instances_by_path = {i.full_path: i for i in instances.values()}
  ...
  def test_get_instances_by_other_region_returns_empty_result(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, locations=['fake-region'])
    instances = gce.get_instances(context)
  ...
  def test_is_gke_node_true(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, labels=DUMMY_INSTANCE3_LABELS)
    instances = gce.get_instances(context)
  ...
  def test_min_cpu_platform(self):
    instance = gce.get_instance(DUMMY_PROJECT_NAME, DUMMY_ZONE, DUMMY_INSTANCE1_NAME)
    self.assertEqual(instance.min_cpu_platform(), 'Intel Skylake')
    instance2 = gce.get_instance(DUMMY_PROJECT_NAME, DUMMY_ZONE, DUMMY_INSTANCE2_NAME)
  ...
  def test_network(self):
    ...
    instances = gce.get_instances(context)
    instances_by_path = {i.full_path: i for i in instances.values()}
    assert instances_by_path[DUMMY_INSTANCE1_PATH].network.name == DUMMY_DEFAULT_NAME
    assert instances_by_path[DUMMY_INSTANCE2_PATH].network.name == DUMMY_DEFAULT_NAME
  ...
  def test_service_account(self):
    ...
    instances_by_path = {i.full_path: i for i in instances.values()}
    assert (
      instances_by_path[DUMMY_INSTANCE1_PATH].service_account
  ...
  def test_get_region_managed_instance_groups(self):
    context = models.Context(project_id=DUMMY_REGION_MIG_PROJECT_NAME, locations=['us-central1'])
    ...
    assert m.template.get_metadata('startup-script-url') == apigee.MIG_STARTUP_SCRIPT_URL
  ...
  def test_managed_instance_group_zone(self):
    ...
    mig1 = gce.ManagedInstanceGroup(
      ...
      {
        'name': 'mig1',
        'zone': 'https://www.googleapis.com/compute/v1/projects/p1/zones/us-central1-a',
        'selfLink': '...',
      },
    )
  ...
  def test_managed_instance_group_get_method(self):
    mig = gce.ManagedInstanceGroup(
      'p1',
      {
        ...
        'zone': 'https://www.googleapis.com/compute/v1/projects/p1/zones/us-central1-a',
        'selfLink': '...',
        'status': {'versionTarget': {'isReached': True}},
      },
    )
  ...
  def test_autoscaler_get_method(self):
    autoscaler = gce.Autoscaler(
      ...
      {
        'name': 'autoscaler1',
        'selfLink': 'https://www.googleapis.com/compute/v1/projects/p1/zones/us-central1-a/'
    ...
    self.assertEqual(
      autoscaler.full_path, 'projects/p1/zones/us-central1-a/autoscalers/autoscaler1'
    )
  ...
  def test_get_instance_templates(self):
    ...
    prefix = 'projects/gcpdiag-gce1-aaaa/global/instanceTemplates/gke-gke1-default-pool'
    ...
    assert any(tag.startswith('gke-') and tag.endswith('-node') for tag in t.tags)
    # service_account
    assert t.service_account == f'{DUMMY_PROJECT_NR}-compute@developer.gserviceaccount.com'

  def test_mig_template(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, labels=DUMMY_INSTANCE2_LABELS)
  ...
  def test_get_instance_interface_effective_firewalls(self):
    ...
    firewalls = gce.get_instance_interface_effective_firewalls(instance=instance, nic='nic0')
  ...
  def test_is_dataproc_instance(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, labels=DATAPROC_LABELS)
    instances = gce.get_instances(context)
    for i in instances.values():
  ...
  def test_get_instances_by_id(self):
    context = models.Context(project_id=DUMMY_PROJECT_NAME, resources=['1010101011'])
    instances = gce.get_instances(context)
