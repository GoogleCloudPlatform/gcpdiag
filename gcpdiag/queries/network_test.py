...
class TestNetwork:
  ...
  def test_subnetworks(self):
    ...
    assert isinstance(net.subnetworks[expected_subnet_url].ip_network, ipaddress.IPv4Network)

  def test_cluster_subnetwork(self):
    subnet = network.get_subnetwork(
      project_id=DUMMY_GKE_PROJECT_ID, region=DUMMY_GKE_REGION, subnetwork_name=DUMMY_GKE_SUBNET
    )
  ...
  def test_cluster_subnetwork_iam_policy(self):
    ...
    policy = network.get_subnetwork_iam_policy(
      context=context, region=DUMMY_GKE_REGION, subnetwork_name=DUMMY_GKE_SUBNET
    )
  ...
  def test_get_routers(self):
    ...
    sub1 = network.get_subnetwork(
      project_id=DUMMY_GKE_PROJECT_ID, region=DUMMY_GKE_REGION, subnetwork_name=DUMMY_GKE_SUBNET
    )
    sub2 = network.get_subnetwork(
      project_id=DUMMY_GKE_PROJECT_ID, region=DUMMY_GKE_REGION, subnetwork_name=DUMMY_DEFAULT_SUBNET
    )
    router = network.get_router(
      project_id=DUMMY_GKE_PROJECT_ID, region=DUMMY_GKE_REGION, network=net
    )
  ...
  def test_ingress_allow_source_sa(self):
    ...
    r = net.firewall.check_connectivity_ingress(
      src_ip=ipaddress.ip_network('10.200.0.16/29'),  #
      source_service_account='service-12340002@compute-system.iam.gserviceaccount.com',
      ip_protocol='tcp',
      port=4000,
    )
    assert r.action == 'allow'
  ...
  def test_ingress_sub_policy_allow_target_sa(self):
    ...
    r = net.firewall.check_connectivity_ingress(
      ...
      ip_protocol='tcp',
      port=2000,
      target_service_account='service-12340002@compute-system.iam.gserviceaccount.com',
    )
    assert r.action == 'allow'
    assert r.matched_by_str == 'policy: sub-folder-policy'
  ...
  def test_get_router_by_name(self):
    router = network.get_router_by_name(
      project_id='gcpdiag-gke1-aaaa', region='europe-west4', router_name='gke-default-router'
    )
    assert router.name == 'gke-default-router'

    router = network.get_router_by_name(
      project_id='gcpdiag-gke1-aaaa', region='us-east4', router_name='dummy-router1'
    )
    assert router.name == 'dummy-router1'

    router = network.get_router_by_name(
      project_id='gcpdiag-gke1-aaaa', region='us-east4', router_name='dummy-router2'
    )
    assert router.name == 'dummy-router2'

    router = network.get_router_by_name(
      project_id='gcpdiag-gke1-aaaa', region='us-east4', router_name='dummy-router3'
    )
    assert router.name == 'dummy-router3'

  def test_bgp_peer_status(self):
    vlan_router_status = network.nat_router_status(
      'gcpdiag-interconnect1-aaaa', router_name='dummy-router1', region='us-east4'
    )
    assert get_path(vlan_router_status.bgp_peer_status[0], ('state'), default=None) == 'Established'
    assert get_path(vlan_router_status.bgp_peer_status[1], ('state'), default=None) == 'Established'

    vlan_router_status = network.nat_router_status(
      'gcpdiag-interconnect1-aaaa', router_name='dummy-router2', region='us-east4'
    )
    assert get_path(vlan_router_status.bgp_peer_status[0], ('state'), default=None) == 'Established'
    assert get_path(vlan_router_status.bgp_peer_status[1], ('state'), default=None) == 'Idle'
  ...
  def test_firewall_policy_sorting_same_ip(self):
    ...
    # same src ip range
    r = net.firewall.check_connectivity_ingress(
      src_ip=ipaddress.ip_network('10.104.0.1/32'), ip_protocol='tcp', port=80
    )
    assert r.action == 'deny'
  ...
  def test_firewall_policy_sorting(self):
    ...
    r = net.firewall.check_connectivity_ingress(
      src_ip=ipaddress.ip_network('10.101.0.1/32'), ip_protocol='tcp', port=2001
    )
    assert r.action == 'allow'
    assert r.matched_by_str == 'policy: parent-folder-policy'
