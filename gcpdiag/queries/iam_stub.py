...
class IamApiStub:
  ...
  def __init__(self, api_project_id: Optional[str] = None, mock_state='init', argument=None):
  ...
  def get(self, name):
    if self.mock_state == 'serviceaccounts':
      ...
      raise ValueError("can't call this method here (mock_state: %s)" % self.mock_state)

  def getIamPolicy(self, resource):
    if self.mock_state == 'serviceaccounts':
      m = re.match(r'projects/[^/]+/serviceAccounts/(.*)', resource)
      return IamApiStub(self.api_project_id, 'serviceaccounts_getIamPolicy', m.group(1))
    else:
      raise ValueError("can't call this method here (mock_state: %s)" % self.mock_state)
  ...
  def execute(self, num_retries=0):
    ...
    if self.mock_state == 'serviceaccounts_get':
      ...
      with open(service_accounts_filename, encoding='utf-8') as json_file:
        ...
        service_accounts = {sa['email']: sa for sa in service_accounts_data['accounts']}
