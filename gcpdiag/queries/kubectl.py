...
class KubectlExecutor:
  ...
  def make_kube_config(self) -> bool:
    ...
    if self.cluster.endpoint is None:
      logging.warning(
        'No kubernetes API server endpoint found for cluster %s', self.cluster.short_path
      )
      return False
    ...
    # The file contains sensitive information, so we need to make sure that it
  ...
  def kubectl_execute(self, command_list: list[str]):
    ...
    """
    res = subprocess.run(command_list, check=False, capture_output=True, text=True)
...
def verify_auth(executor: KubectlExecutor) -> bool:
  ...
  if stderr:
    logging.warning(
      'Failed to authenticate kubectl for cluster %s: %s',
      executor.cluster.short_path,
      stderr.strip('\n'),
    )
    return False
  return True
...
def clean_up():
  ...
  try:
    ...
    logging.debug(
      'Error cleaning up kubeconfig file used by gcpdiag: %s: %s', type(err).__name__, err
    )
