...
class TestGcpdiagSearchCommand(unittest.TestCase):
  ...
  def test_search_rules_output(self, mock_rank_lint_rules, mock_rank_runbook_rules, mock_print):
...
class TestRankingFunctions(unittest.TestCase):
  ...
  def setUp(self):
    self.runbook_rules = {'rule1': self.TestDT1, 'rule2': self.TestDT2, 'rule3': self.TestDT3}
  ...
  def test_rank_runbook_rules_frequency(self):
    ...
    args = Namespace(search=['issue-kw', 'public'], limit_per_type=2, product=['search'])
    ranked_rules = search_cmd._rank_runbook_rules(self.runbook_rules, args)
  ...
  def test_rank_lint_rules_frequency(self):
    ...
    args = Namespace(search=['issue-kw', 'issue-kw2'], limit_per_type=2, product=['prod'])
  ...
  def test_rank_lint_rules_max_heap(self):
    ...
    args = Namespace(search=['lint', 'rule'], limit_per_type=3, product=['prod', 'another-prod'])
    ranked_rules = search_cmd._rank_lint_rules(self.lint_rules, args)
