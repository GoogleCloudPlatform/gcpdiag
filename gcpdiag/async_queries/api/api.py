...
class API:
  ...
  def __init__(self, creds: Creds, retry_strategy: RetryStrategy, sleeper: Sleeper) -> None:
    self._creds = creds
  ...
  async def call(self, method: str, url: str, json: Optional[Any] = None) -> Any:
    for timeout in self._retry_strategy.get_sleep_intervals():
      async with aiohttp.request(method, url, headers=self._get_headers(), json=json) as resp:
        if resp.status == 200:
