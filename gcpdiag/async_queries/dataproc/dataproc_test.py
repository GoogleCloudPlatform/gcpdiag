...
class TestProjectRegions(unittest.IsolatedAsyncioTestCase):
  ...
  def setUp(self) -> None:
    self.project_regions = FakeProjectRegions(regions=['westeros-central1', 'essos-east2'])
    self.westeros_list_call = fake_api.APICall(
      method='GET',
      url=(
        'https://dataproc.googleapis.com/v1/projects/test-project/regions/'
        'westeros-central1/clusters'
      ),
    )
    self.essos_list_call = fake_api.APICall(
      'GET', 'https://dataproc.googleapis.com/v1/projects/test-project/regions/essos-east2/clusters'
    )

    self.api = fake_api.FakeAPI(
      responses=[
        (
          self.westeros_list_call,
          yaml.safe_load("""
        clusters:
    ...
    self.dataproc = dataproc.Dataproc(
      api=self.api, project_id='test-project', project_regions=self.project_regions
    )
  ...
  async def test_cluster_status(self) -> None:
    westeros1, westeros2, essos1 = await asyncio.gather(
      *[self.dataproc.get_cluster_by_name(n) for n in ['westeros1', 'westeros2', 'essos1']]
    )
  ...
  async def test_deduplication(self) -> None:
    await asyncio.gather(
      self.dataproc.get_cluster_by_name('westeros1'), self.dataproc.get_cluster_by_name('westeros1')
    )
