  ...
  def execute(self):
    ...
    try:
      ...
      if tunnel.status == 'ESTABLISHED':
        op.add_ok(tunnel, reason=op.prep_msg(op.SUCCESS_REASON, tunnel=tunnel, project=project_id))
      else:
        op.add_failed(
          tunnel,
          reason=op.prep_msg(
          ...
          remediation=op.prep_msg(
            op.FAILURE_REMEDIATION, tunnel=tunnel, project=project_id, status=tunnel.status
          ),
        )
      return True
      ...
      op.add_skipped(
        ...
        reason=op.prep_msg(
          ...
          project=project_id,
          issue=(
            f'API call failed {e} : Check that the provided '
...
def _get_combined_metric_data(project_id, query_ingress, query_egress, start_time, end_time):
  """Helper to fetch and merge ingress/egress metrics by timestamp."""
  try:
    m_ingress = monitoring.queryrange(project_id, query_ingress, start_time, end_time)
  except (gapi_errors.HttpError, monitoring.GcpApiError) as e:
    op.warning(f'Failed to query ingress metrics: {e}')
    m_ingress = {}
  try:
    m_egress = monitoring.queryrange(project_id, query_egress, start_time, end_time)
  except (gapi_errors.HttpError, monitoring.GcpApiError) as e:
  ...
  for ts in all_timestamps:
    ...
    combined.append(
      {'timestamp': datetime.fromtimestamp(ts).isoformat(), 'total': val_in + val_out}
    )
  return combined
  ...
  def execute(self):
    ...
    try:
      ...
      op.add_skipped(
        None,
        reason=op.prep_msg(
          op.SKIPPED_REASON, project=project_id, issue=f'Failed to get tunnel: {e}'
        ),
      )
      return
    ...
    if tunnel.status != 'ESTABLISHED':
      op.add_skipped(
        tunnel,
        reason=op.prep_msg(op.SKIPPED_REASON, project=project_id, issue='Tunnel not Established'),
      )
      return
    ...
    combined_data = _get_combined_metric_data(
      project_id, q_in, q_out, op.get(flags.START_TIME), op.get(flags.END_TIME)
    )
    ...
    if not combined_data:
      op.add_failed(
        tunnel,
        reason=op.prep_msg(
          op.FAILURE_REASON, tunnel=tunnel, project=project_id, issue='has no traffic'
        ),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION, remediations='Check route configuration'),
      )
      return

    exceeded = [d for d in combined_data if d['total'] >= constants.PACKET_PER_SECOND_LIMIT]
    ...
    if exceeded:
      op.add_failed(
        ...
        remediation=op.prep_msg(op.FAILURE_REMEDIATION, remediations='Reduce rate or add tunnels'),
      )
    else:
      op.add_ok(tunnel, reason=op.prep_msg(op.SUCCESS_REASON, tunnel=tunnel, project=project_id))
...
class TunnelPacketsDropCheck(runbook.Step):
  ...
  DROP_REASONS = {
    'dont_fragment_icmp': {
      'reason': 'Dropped ICMP packet larger than MTU with DF bit set (PMTUD).',
      'remediation': 'Ensure ICMP packets for PMTUD do not exceed effective MTU.',
    },
    'dont_fragment_nonfirst_fragment': {
      'reason': 'Non-first fragment of UDP/ESP packet exceeds MTU with DF bit set.',
      'remediation': 'Verify application fragmentation handling or reduce IP packet size.',
    },
    'exceeds_mtu': {
      'reason': 'First fragment of UDP/ESP packet exceeds MTU with DF bit set.',
      'remediation': 'Reduce effective MTU (usually 1460 bytes) or permit fragmentation.',
    },
    'invalid': {
      'reason': 'Dropped due to invalid state (corruption or unexpected sequence).',
      'remediation': 'Review peer VPN config and capture traffic to check for corruption.',
    },
    'sa_expired': {
      'reason': 'Packet used unknown or expired SA (negotiation failure).',
      'remediation': 'Sync IKE/IPsec SA lifetimes on both peers; consider a tunnel reset.',
    },
    'sequence_number_lost': {
      'reason': 'Sequence number significantly larger than expected (packet loss).',
      'remediation': 'Investigate network path for congestion or connectivity issues.',
    },
    'suspected_replay': {
      'reason': 'ESP packet received with a sequence number already processed.',
      'remediation': 'Check for network devices (load balancers) duplicating or reordering packets.',
    },
    'throttled': {
      'reason': 'Packet dropped due to excessive load on Cloud VPN gateway.',
      'remediation': 'Reduce traffic or provision additional tunnels/Cloud Interconnect.',
    },
    'unknown': {
      'reason': 'Packet dropped for an uncategorized reason.',
      'remediation': 'Collect logs from both peers and contact Google Cloud support.',
    },
  }
  ...
  def execute(self):
    ...
    if tunnel.status != 'ESTABLISHED':
      op.add_skipped(
        tunnel,
        reason=op.prep_msg(
          op.SKIPPED_REASON, project=project_id, issue='Check Skipped: Tunnel not Established'
        ),
      )
      return

    issue_found = False
    flows = [('egress', 'sent_packets_count'), ('ingress', 'received_packets_count')]
    ...
    for direction_label, metric_suffix in flows:
      ...
      try:
        status_drop = monitoring.queryrange(project_id, query, start_time, end_time)
      except (gapi_errors.HttpError, monitoring.GcpApiError) as e:
        op.warning(f'Failed to query packet drop metrics for {direction_label}: {e}')
        continue
      for metric_data in status_drop.get('data', {}).get('result', []):
        ...
        drops = [
          [datetime.fromtimestamp(t).isoformat(), float(v)]
          for t, v in metric_data['values']
          if float(v) >= 1
        ]
        ...
        if drops:
          ...
          op.add_failed(
            tunnel,
            reason=op.prep_msg(
              op.FAILURE_REASON, tunnel=tunnel, project=project_id, issue=base_issue
            ),
            remediation=op.prep_msg(
...
class TunnelDownStatusReason(runbook.Step):
  ...
  def execute(self):
    ...
    log_text = ' '.join([log.message for log in fetched_logs])
    found_issue = False
    ...
    if 'establishing IKE_SA failed' in log_text:
      ...
      op.add_failed(
        ...
        reason=op.prep_msg(
          ...
          tunnel=tunnel,
          project=project_id,
          issue='IKE_SA failure: Peer not responding. Possible firewall blocking traffic.',
        ),
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION,
          remediations='Ensure UDP ports 500/4500 are open, and check peer reachability.',
        ),
      )
    ...
    if 'Remote traffic selectors narrowed' in log_text:
      ...
      op.add_failed(
        ...
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION,
          remediations="Match Cloud VPN remote selectors with peer's local selectors.",
        ),
      )
    ...
    if 'Local traffic selectors narrowed' in log_text:
      ...
      op.add_failed(
        ...
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION,
          remediations="Ensure Cloud VPN local selectors match peer's remote selectors.",
        ),
      )
    ...
    if 'Proposal mismatch in CHILD SA' in log_text:
      ...
      op.add_failed(
        ...
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION,
          remediations='Align Phase 2 settings (Encryption/DH groups) on both ends.',
        ),
      )
    ...
    if 'VPN Task maintenance' in log_text:
      ...
      op.add_failed(
        ...
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION,
          remediations='Tunnels usually recover automatically. Contact Support if it stays down.',
        ),
      )
      ...
      if tunnel.status != 'ESTABLISHED':
        op.add_failed(
          ...
          reason=op.prep_msg(
            ...
            tunnel=tunnel,
            project=project_id,
            issue='Tunnel is not established, but no specific error logs were found.',
          ),
          remediation=op.prep_msg(
            op.FAILURE_REMEDIATION,
            remediations='Try broadening the timeframe or checking the peer device logs.',
          ),
        )
      else:
        op.add_ok(tunnel, reason=op.prep_msg(op.SUCCESS_REASON, tunnel=tunnel, project=project_id))
