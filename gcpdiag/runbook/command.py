    ...
    if values:
      ...
      for file_path in values:
        try:
          ...
          parser.error(f'argument {option_string} expected key:value, received {file_path}')
...
def expand_and_validate_path(arg) -> str:
  ...
  if bool(os.getenv('CLOUD_SHELL')):
    ...
    # User supplied path
      ...
      raise argparse.ArgumentTypeError(f'The {arg} folder must be located in your home directory')
...
def validate_args(args):
  if args.runbook is None and not args.bundle_spec:
    print('Error: Provide a runbook id  or "--bundle-spec=YAML_FILE_PATH" must be provided.')
...
def _init_runbook_args_parser():
  ...
  parser.add_argument(
    'runbook',
    help='Runbook to execute in the format product/runbook-name or product/name',
    nargs='?',
  )
  ...
  parser.add_argument(
    ...
    '--auto',
    help=(
      'Execute runbook autonomously. Use this to skip human tasks. '
...
def _load_bundles_spec(file_path):
  ...
  if os.path.exists(file_path):
    ...
    print(f'ERROR: Bundle Specification file: {file_path} does not exist!', file=sys.stderr)
    sys.exit(1)
...
def run(argv) -> None:
  ...
  except (DiagnosticTreeNotFoundError, AttributeError, ValueError, KeyError) as e:
    logging.error(e)
