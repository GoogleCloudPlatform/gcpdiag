...
class RunPermissionChecks(runbook.Gateway):
  ...
  def execute(self):
    ...
    try:
      ...
      op.add_skipped(
        project_placeholder,
        reason="Permission check is being skipped because it's not possible to successfully identify"
    ...
    try:
      ...
      if 'not found or deleted' in err.message:
        op.add_skipped(
          project_placeholder,
          reason=(
            f'Project "{project_placeholder.project_id}" not found or'
            ' deleted. Provide a valid project identifier'
          ),
        )
        self.add_child(bq_queries.BigQueryEnd())
        return
      elif 'Caller does not have required permission to use project' in err.message:
        op.add_skipped(
          project_placeholder,
          reason=(
    ...
    if skip_permission_check:
      ...
      op.add_skipped(
        project,
        reason=f'Permission check is being skipped because --skip_permission_check=True was used.\
    ...
    if self.runbook_id != 'Failed Query Runbook':
      if organization_id:
        try:
          host_project_context = op.get_context().copy_with(project_id=organization_id)
    ...
    if project_policy is None:
      op.add_skipped(
        resource=project,
        reason=f'The permission check step will not be carried out.\nReason: {self.principal_email} '
      ...
      if reqs:
        ...
        if mandatory_reqs:
          ...
          missing_mandatory = bq_queries.get_missing_permissions(mandatory_reqs, mandatory_perms)
          ...
          if missing_mandatory:
            op.add_failed(
              ...
              remediation=op.prep_msg(op.FAILURE_REMEDIATION, principal=self.principal_email),
            )
          ...
          if organization_policy is not None:
            ...
            for perm in optional_org_reqs:
              ...
              if not org_perms.get(perm):
                self._add_info(
                  insight_name=f'Organization-level analysis requiring: {perm}',
                  missing_permissions={perm},
                  principal=self.principal_email,
                  is_org=True,
                )
        ...
        op.add_ok(resource=project, reason='All permission checks are complete.')

  def _get_principal(self):
    principal_email_doman = self.principal_email.partition('@')[2]
    if (
      principal_email_doman in SERVICE_AGENT_DOMAINS
      or principal_email_doman.startswith('gcp-sa-')
      or self.principal_email.endswith(DEFAULT_SERVICE_ACCOUNT_DOMAINS[1])
    ):
