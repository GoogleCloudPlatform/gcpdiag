...
ERROR_MAP: Dict[
  ...
  ('Concurrent jobs in the same session are not allowed',): {
    'cause': (
      'This error can occur when multiple queries are running'
  ...
  (
    ...
    'cause': ('The HSM symmetric cryptographic requests per region quota is being exceeded.'),
    'remediation': (
  ...
  ('Already Exists: Job',): {
    'cause': ('You are trying to create a job with a job id that already exists.'),
    'remediation': (
      'Allow BigQuery to generate a random jobId value instead of specifying one explicitly.'
    ),
  },
  (
    'Dataset',
  ...
  (
    ...
    ('Top memory consumer(s): sort operation used for table update (UPDATE/DELETE/MERGE)'),
  ): {
    'cause': (
  ...
  (
    ...
    'remediation': (
      ' - Use a LIMIT clause to reduce the result set.'
  ...
  (
    ...
    'cause': ('For SELECT statements, this error occurs when the query uses too many resources.'),
    'remediation': (
  ...
  (
    ...
    'cause': (
      'You are attempting to queue more interactive or batch queries than the queue limit allows. '
    ),
    'remediation': (
  ...
  (
    ...
    'cause': (
      ' Your column-partitioned table reaches the quota of the'
  ...
  (
    ...
    'cause': ' BigQuery returns this error message when your table reaches the'
    ...
    'remediation': ' This quota cannot be increased. To remediate, do the following:'
    ' \n - If you frequently load data from multiple small files stored'
    ' in Cloud Storage that uses a job per file, then combine multiple'
    ' load jobs into a single job. You can load from multiple Cloud'
    ' Storage URIs with a comma-separated list (for example,'
  ...
  (
    ...
    'cause': ('The export exceeds the default 50 TiB (Tebibytes) daily limit in a project'),
    'remediation': (
  ...
  ('too many concurrent queries with remote functions',): {
    'cause': (' The number of concurrent queries that contain remote functions exceeds the limit.'),
    'remediation': (
  ...
  ('User requested cancellation',): {
    'cause': (
      'The job was manually stopped by a user request before it'
  ...
  (
    ...
    'cause': ('User does not have sufficient permissions in the mentioned project.'),
    'remediation': (
  ...
  (
    ...
    'cause': 'There are three main reasons why this error might occur:'
