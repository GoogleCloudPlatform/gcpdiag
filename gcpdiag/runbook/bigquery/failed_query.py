...
class FailedQuery(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    flags.BQ_SKIP_PERMISSION_CHECK: {
      'type': bool,
      'help': 'Indicates whether to skip the permission check to speed up the investigation.',
      'required': False,
      'default': False,
    },
  }
...
class BigQueryFailedQueryStart(runbook.StartStep):
  ...
  def execute(self):
    ...
    project_placeholder = crm.Project(
      resource_data={'name': 'projects/0000000000000', 'projectId': project_id}
    )
    user_email = ''
    try:
      ...
      if job_region.lower() in bigquery.BIGQUERY_REGIONS:
        if job_id:
          ...
          op.add_skipped(
            ...
            reason='Invalid job identifier provided - a job identifier cannot be an empty string.',
          )
          return
      else:
        op.add_skipped(
          resource=project_placeholder,
          reason='Invalid job region provided. Please provide a valid BigQuery region.',
        )
        return
    else:
      op.add_skipped(
        resource=project_placeholder,
        reason='Invalid project identifier provided - the project identifier cannot be an empty string.',
      )
      return
    ...
    try:
      ...
      if not project:
        op.add_skipped(
          project_placeholder,
          reason=f'Project "{project_id}" not found or you lack access permissions',
        )
        return
      ...
      if 'not found or deleted' in err.message.lower():
        op.add_skipped(
          project_placeholder,
          reason=f'Project "{project_id}" not found or deleted. Provide a valid project identifier',
        )
        return
        ...
        'resourcemanager.projects.get' in err.message.lower()
        and 'denied on resource' in err.message.lower()
      ):
        op.info(
    ...
    try:
      ...
      if not is_bq_api_enabled:
        op.add_skipped(
          project,
          reason=f'BigQuery API is not enabled in project {project_id}. Terminating investigation.',
        )
        return
      else:
        op.info('The BigQuery API is enabled.')
    except utils.GcpApiError as err:
      if 'access' in err.message.lower() or 'permission denied' in err.message.lower():
        op.info(
...
class BigQueryJobExists(runbook.Gateway):
  ...
  def execute(self):
    ...
    project_id = op.get(flags.PROJECT_ID)
    project = crm.Project(resource_data={'name': 'projects/0000000000000', 'projectId': project_id})
    try:
      project = bigquery.get_bigquery_project(project_id)
    ...
    try:
      ...
      if 'not found' in err.message.lower():
        ...
        try:
          ...
          op.info(
            message='Unable to fetch user email. Please make sure to authenticate properly'
...
class ConfirmBQJobIsDone(runbook.Gateway):
  ...
  def execute(self):
    ...
    if not job:
      op.add_skipped(op.get(flags.PROJECT_ID), reason='Cannot retrieve job details.')
      return
    if job.job_state == 'DONE':
      ...
      op.add_failed(
        job,
        reason=op.prep_msg(op.FAILURE_REASON, job_id=job.id, job_state=job.job_state),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION),
      )
      self.add_child(BigQueryEnd())
...
class CheckBQJobHasFailed(runbook.Gateway):
  ...
  def execute(self):
    ...
    if job.job_error_result:
      ...
      skip_reason = (
        op.prep_msg(op.FAILURE_REASON, job_id=job.id) + '\n' + op.prep_msg(op.FAILURE_REMEDIATION)
      )
...
class BigQueryErrorIdentification(runbook.Step):
  ...
  def execute(self):
    ...
    if not job or not job.job_error_result:
      op.add_skipped(
        op.get(flags.PROJECT_ID), reason='Cannot retrieve job error details for analysis.'
      )
      return
    unique_errors = set()
    ...
    if not unique_errors:
      op.add_uncertain(job, reason='The job failed but returned no error message to analyze.')
      return
    ...
    for error_text in unique_errors:
      ...
      for error_substrings, details in constants.ERROR_MAP.items():
        error_substrings_lower = [error_substring.lower() for error_substring in error_substrings]
        if all(
          error_substring_lower in error_message_searchable
          ...
          op.add_failed(
            ...
            remediation=op.prep_msg(op.FAILURE_REMEDIATION, remediation=details['remediation']),
          )
          break
    if not has_matched:
