...
class Operator:
  ...
  def create_context(
    self, project_id, locations=None, labels=None, parameters=None, resources=None
  ):
...
def prompt(message: str, kind: str = '', options: dict = None, choice_msg: str = '') -> Any:
  """
  ...
  """
  operator = _get_operator()
  return operator.interface.prompt(
    message=message, kind=kind, options=options, choice_msg=choice_msg
  )
...
def info(message: str, step_type='INFO') -> None:
  ...
  operator.interface.rm.add_step_info_metadata(
    run_id=operator.run_id, step_execution_id=operator.step.execution_id, value=message
  )


def prep_rca(resource: Optional[models.Resource], template, suffix, kwarg) -> None:
  """Parses a log form and complex Jinja templates for root cause analysis (RCA)."""
  operator = _get_operator()
  return operator.interface.prepare_rca(
...
def add_skipped(resource: Optional[models.Resource], reason: str) -> None:
  ...
  operator = _get_operator()
  with report_lock:
    operator.interface.add_skipped(
      run_id=operator.run_id,
      resource=resource,
      reason=reason,
...
def add_uncertain(
  resource: models.Resource, reason: str, remediation: str = None, human_task_msg: str = ''
) -> Any:
...
def get_step_outcome(execution_id) -> Tuple[Any, dict]:
  ...
  operator = _get_operator()
  with report_lock:
    step_result = operator.interface.rm.reports[operator.run_id].results.get(execution_id)
  if not step_result:
    return (None, {})
...
def add_metadata(key, value):
  ...
  with report_lock:
    operator.interface.rm.add_step_metadata(
      run_id=operator.run_id, step_execution_id=operator.step.execution_id, key=key, value=value
    )
