...
class BigquerySubscriptionDelivery(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    flags.SUBSCRIPTION_NAME: {'type': str, 'help': 'The Pub/Sub subscription ID', 'required': True},
    'table_id': {
      'type': str,
      'help': ('The BigQuery table ID in the format "project_id:dataset.table" or "dataset.table"'),
      'required': True,
    },
  }
  ...
  def build_tree(self):
    ...
    subscription_status_check = SubscriptionStatusCheck(is_push_subscription=True)
...
class StartStep(runbook.StartStep):
  ...
  def execute(self):
    ...
    try:
      ...
      op.add_failed(
        resource=None,
        reason=op.prep_msg(op.FAILURE_REASON, project_id=op.get(flags.PROJECT_ID)),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION),
      )
      return
  ...
  def execute(self):
    ...
    try:
      subscription = pubsub.get_subscription(
        op.get(flags.PROJECT_ID), op.get(flags.SUBSCRIPTION_NAME)
      )
    except utils.GcpApiError as err:
      if 'Resource not found' in err.message:
        op.add_failed(
          resource=subscription,
          reason=op.prep_msg(op.FAILURE_REASON, subscription_name=op.get(flags.SUBSCRIPTION_NAME)),
          remediation=op.prep_msg(op.FAILURE_REMEDIATION),
        )
        return
      else:
    ...
    if not subscription:
      ...
      op.add_ok(subscription, reason=f"Pub/Sub subscription '{subscription.name}' found.")
...
class BigQueryTableExistenceCheck(runbook.Step):
  ...
  def execute(self):
    ...
    try:
      ...
      op.add_failed(
        resource=None,
        reason=op.prep_msg(op.FAILURE_REASON, table_full_id=table_full_id),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION),
      )
      return
...
class BigQueryWriterPermissionCheck(runbook.Step):
  ...
  def execute(self):
    ...
    required_permissions = {'bigquery.tables.updateData', 'bigquery.tables.getData'}
    has_permissions = all(
      project_policy.has_permission(service_account, p) for p in required_permissions
    )

    if not has_permissions:
      op.add_failed(
        resource=None,
        reason=op.prep_msg(op.FAILURE_REASON, service_account=service_account, project_id=project),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION, service_account=service_account),
      )
    else:
      op.add_ok(
        resource=None,
        reason='Pub/Sub service account has permissions to write to the BigQuery table.',
      )
...
class SubscriptionStatusCheck(runbook.Step):
  ...
  def execute(self):
    ...
    if subscription.is_active():
      op.add_ok(resource=subscription, reason=f'The subscription {subscription} state is ACTIVE.')
    else:
      op.add_failed(
        resource=subscription,
  ...
  def execute(self):
    ...
    if not push_metric:
      op.add_skipped(
        ...
        reason=('Skipping as no traffic delivery to the BigQuery has been detected'),
      )
    else:
      subscription = pubsub.get_subscription(
        project_id=project_id, subscription_name=op.get(flags.SUBSCRIPTION_NAME)
      ...
      if not found_error_response:
        op.add_ok(resource=subscription, reason='No error responses from BigQuery detected')
