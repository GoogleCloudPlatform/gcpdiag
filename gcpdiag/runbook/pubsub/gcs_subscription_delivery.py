...
class CheckServiceAccountPermissions(runbook.Step):
  ...
  def execute(self):
    ...
    project_id = op.get(flags.PROJECT_ID)
    subscription = pubsub.get_subscription(project_id, op.get(flags.SUBSCRIPTION_NAME))
    bucket_name = subscription.gcs_subscription_bucket()
    context = models.Context(project_id=project_id)

    if not bucket_name:
      op.add_skipped(None, reason='Bucket name not found, skipping permission checks.')
      return
    ...
    try:
      ...
      has_permissions = all(policy.has_permission(service_account, p) for p in required_permissions)

    except utils.GcpApiError as e:
      op.add_uncertain(
        subscription,
        reason=f'Could not verify IAM permissions on bucket {bucket_name}. Error: {e}',
        remediation=(
    ...
    if has_permissions:
      ...
      op.add_failed(
        resource=None,
        reason=op.prep_msg(
          op.FAILURE_REASON, service_account=service_account, bucket_name=bucket_name
        ),
        remediation=op.prep_msg(
          op.FAILURE_REMEDIATION, service_account=service_account, bucket_name=bucket_name
        ),
      )
...
class GcsSubscriptionDeliveryEnd(runbook.EndStep):
  ...
  def execute(self):
    ...
    subscription_id = op.get(flags.SUBSCRIPTION_NAME)
    op.info(
      f'Finished checks for Pub/Sub to GCS subscription{subscription_id} in project {project_id}.'
    )
    op.info(
