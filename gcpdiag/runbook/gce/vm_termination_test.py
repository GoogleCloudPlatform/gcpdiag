...
class VmTerminationTestBase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.params = {
      ...
      flags.PROJECT_NUMBER: DUMMY_PROJECT_NUMBER,
      flags.INSTANCE_ID: '111222333',
      flags.INSTANCE_NAME: 'test-instance',
      flags.ZONE: 'us-central1-a',
      flags.START_TIME: datetime.datetime(2025, 3, 18, tzinfo=datetime.timezone.utc),
      flags.END_TIME: datetime.datetime(2025, 3, 19, tzinfo=datetime.timezone.utc),
    }
    ...
    self.mock_get_instance = self.enterContext(
      mock.patch('gcpdiag.queries.gce.get_instance', return_value=self.mock_vm)
    )
...
class TerminationOperationTypeTest(VmTerminationTestBase):
  ...
  def test_host_error_operation(self):
    self.gce_get_ops_patch.return_value = [make_operation(constants.HOST_ERROR_METHOD)]
  ...
  def test_stop_operation(self):
    ...
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
      assert any(isinstance(c, vm_termination.StopOperationGateway) for c in step.steps)

  def test_preempted_operation(self):
    self.gce_get_ops_patch.return_value = [make_operation(constants.INSTANCE_PREMPTION_METHOD)]
    step = vm_termination.TerminationOperationType()
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
      assert any(isinstance(c, vm_termination.PreemptibleInstance) for c in step.steps)

  def test_shutdown_operation(self):
    self.gce_get_ops_patch.return_value = [make_operation('compute.instances.guestTerminate')]
    step = vm_termination.TerminationOperationType()
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
      assert any(isinstance(c, vm_termination.GuestOsIssuedShutdown) for c in step.steps)

  def test_mig_repair_operation(self):
    self.gce_get_ops_patch.return_value = [make_operation(constants.IG_INSTANCE_REPAIR_METHOD)]
    step = vm_termination.TerminationOperationType()
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
      assert any(isinstance(c, vm_termination.ManagedInstanceGroupRecreation) for c in step.steps)
...
class TestVmTerminationStubs(VmTerminationTestBase):
  ...
  def test_ccm_termination_api_error(self):
    type(self.mock_vm).get_network_interfaces = mock.PropertyMock(
      side_effect=googleapiclient.errors.HttpError(mock.Mock(status=403), b'Permission denied')
    )
  ...
  def test_stop_gateway_scheduled_policy(self):
    ...
    with op.operator_context(self.operator):
      with mock.patch(
        ...
        return_value=[{'protoPayload': {'methodName': 'ScheduledVMs'}}],
      ):
        self.operator.set_step(step)
        step.execute()
        assert any(isinstance(c, vm_termination.ScheduledStopPolicy) for c in step.steps)
  ...
  def test_start_step_instance_found_by_id(self):
    ...
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
    self.assertEqual(self.operator.parameters[flags.INSTANCE_NAME], self.mock_vm.name)
  ...
  def test_start_step_instance_found_by_name(self):
    ...
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
    self.assertEqual(self.operator.parameters[flags.INSTANCE_ID], self.mock_vm.id)
  ...
  def test_ccm_termination_shared_vpc(self):
    self.mock_vm.get_network_interfaces = [
      {
        'network': 'https://www.googleapis.com/compute/v1/projects/shared-vpc-host/global/networks/vpc-1'
      }
    ]
    step = vm_termination.ComputeClusterManagerTermination()
    step.stop_account = constants.GCE_CLUSTER_MANAGER_EMAIL
  ...
  def test_stop_gateway_user_stop(self):
    ...
    with op.operator_context(self.operator):
      ...
      step.execute()
      self.assertTrue(
        any(isinstance(c, vm_termination.UserOrServiceAccountInitiatedStop) for c in step.steps)
      )
  ...
  def test_stop_gateway_ccm_termination(self):
    ...
    with op.operator_context(self.operator):
      ...
      step.execute()
      self.assertTrue(
        any(isinstance(c, vm_termination.ComputeClusterManagerTermination) for c in step.steps)
      )
