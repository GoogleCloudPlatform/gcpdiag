...
from gcpdiag.runbook.gce import flags, ops_agent
...
class OpsAgentUnitTest(unittest.TestCase):
  def setUp(self):
    ...
    self.mock_get_instance = self.enterContext(mock.patch('gcpdiag.queries.gce.get_instance'))
    self.mock_iam_get_sa_list = self.enterContext(
      mock.patch('gcpdiag.queries.iam.get_service_account_list')
    )
    self.mock_logs_query = self.enterContext(mock.patch('gcpdiag.queries.logs.realtime_query'))
  ...
  def test_vm_has_sa_no_gac_with_instance_sa(self):
    ...
    self.assertEqual(
      self.operator.parameters[flags.SERVICE_ACCOUNT], 'test@iam.gserviceaccount.com'
    )

  def test_vm_has_sa_gac_found(self):
    self.operator.parameters[ops_agent.GAC_SERVICE_ACCOUNT] = (
    ...
    self.mock_interface.add_ok.assert_called_once()
    self.assertEqual(
      self.operator.parameters[flags.SERVICE_ACCOUNT], 'gac-sa@project.iam.gserviceaccount.com'
    )
  ...
  def test_investigate_logging_monitoring_all_enabled(self):
    ...
    self.assertIsInstance(logging_api_step.steps[2], ops_agent.CheckSerialPortLogging)
    self.assertIsInstance(logging_api_step.steps[1].steps[0], gce_gs.VmHasOpsAgent)
    ...
    self.assertIsInstance(monitoring_api_step.steps[1].steps[0], gce_gs.VmHasOpsAgent)
  ...
  def test_end_step_serial_logs_found(self):
    ...
    self.mock_interface.info.assert_called_with(
      'There are new logs indicating ops agent is exporting serial logs', step_type='INFO'
    )

  def test_end_step_metrics_found(self):
    self.mock_logs_query.return_value = []
    self.mock_monitoring_query.return_value = {
      'entry1': {'labels': {'metric.version': 'google-cloud-ops-agent-metrics-1'}}
    }
    step = ops_agent.OpsAgentEnd()
    self.operator.set_step(step)
    step.execute()
  ...
  def test_end_step_no_logs_metrics_prompt_no(self):
    self.mock_logs_query.return_value = []
    self.mock_monitoring_query.return_value = {}
    with mock.patch('gcpdiag.runbook.op.prompt', return_value=op.NO) as mock_prompt:
      step = ops_agent.OpsAgentEnd()
      self.operator.set_step(step)
      step.execute()
      mock_prompt.assert_called_once()
      self.mock_interface.info.assert_called_with(message=op.END_MESSAGE, step_type='INFO')
  ...
  def test_ops_agent_tree_structure(self):
    ...
    def assert_relationship(parent_type, child_type):
      ...
      self.assertTrue(
        found,
        f'Relationship {parent_type.__name__} -> {child_type.__name__} not found',
      )

    with op.operator_context(self.operator):
      assert_relationship(ops_agent.OpsAgentStart, ops_agent.VmHasAServiceAccount)
      assert_relationship(ops_agent.VmHasAServiceAccount, iam_gs.VmHasAnActiveServiceAccount)
      assert_relationship(ops_agent.VmHasAServiceAccount, ops_agent.InvestigateLoggingMonitoring)
