...
class VmPerformance(runbook.DiagnosticTree):
  ...
  parameters = {
    flags.PROJECT_ID: {
      'type': str,
      'help': ('The Project ID associated with the VM having performance issues.'),
      'required': True,
    },
    flags.NAME: {
      'type': str,
      'help': 'The name of the VM having performance issues. Or provide the id i.e -p name=<int>',
      'deprecated': True,
    ...
    flags.ZONE: {
      'type': str,
      'help': 'The Google Cloud zone where the VM having performance issues, is located.',
      'required': True,
    },
    flags.START_TIME: {
      'type': datetime,
      'help': 'The start window(in UTC) to investigate vm performance issues.'
      'Format: YYYY-MM-DDTHH:MM:SSZ',
    },
    flags.END_TIME: {
      'type': datetime,
      'help': 'The end window(in UTC) for the investigation. Format: YYYY-MM-DDTHH:MM:SSZ',
    },
  }
  ...
  def execute(self):
    ...
    if log_entries:
      for log in log_entries:
        start_time_val = datetime.strptime(log['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime(
          '%Y/%m/%d %H:%M:%S'
        )
  ...
  def execute(self):
    ...
    for disk in vm.disks:
      ...
      if pd_health_metrics:
        op.add_failed(
          vm,
          reason=op.prep_msg(op.FAILURE_REASON, disk_name=disk['deviceName']),
          remediation=op.prep_msg(op.FAILURE_REMEDIATION),
        )
      else:
        op.add_ok(vm, reason=op.prep_msg(op.SUCCESS_REASON, disk_name=disk['deviceName']))
...
class CpuOvercommitmentCheck(runbook.Step):
  ...
  def execute(self):
    ...
    if vm.is_sole_tenant_vm or 'e2' in vm.machine_type():
      start_dt_pst = datetime.strptime(vm.laststarttimestamp(), '%Y-%m-%dT%H:%M:%S.%f%z')
      start_dt_utc = start_dt_pst.astimezone(timezone.utc)
      start_dt_utc_plus_5_mins = start_dt_utc + timedelta(minutes=5)
      current_time_utc = datetime.now(timezone.utc)
      within_hours = 9
      if (
        start_dt_utc_plus_5_mins > current_time_utc or not vm.is_running
      ) and vm.laststoptimestamp():
        ...
        stop_dt_pst = datetime.strptime(vm.laststoptimestamp(), '%Y-%m-%dT%H:%M:%S.%f%z')
      ...
      try:
        ...
        op.add_failed(
          op.get(flags.PROJECT_ID),
          reason=('Not able to pull CPU count for instance {}').format(op.get(flags.INSTANCE_NAME)),
          remediation='',
        )
      else:
        if cpu_count_query:
      ...
      if cpu_overcomit_metrics:
        op.add_failed(
          vm,
          reason=('CPU for the VM {} is over committed beyond acceptable limits: {} ms/s').format(
            vm.name, utilization_threshold
          ),
          remediation='',
        )
      else:
...
class DiskAvgIOLatencyCheck(runbook.Step):
  ...
  def execute(self):
    ...
    disk_list = gce.get_all_disks_of_instance(op.get_context(), vm.zone, vm.name)
    disk: gce.Disk
    for disks in disk_list.items():
      ...
      if disk.type in ['pd-balanced', 'pd-ssd', 'pd-standard', 'pd-extreme']:
        ...
        disk_io_latency = monitoring.query(
          ...
          | {}
          """.format(vm.id, disk.name, disk.type, IO_LATENCY_THRESHOLD, within_str),
        )

        if disk_io_latency:
          op.add_failed(
            vm,
            reason=op.prep_msg(op.FAILURE_REASON, disk_name=disk.name),
            remediation=op.prep_msg(op.FAILURE_REMEDIATION),
          )
        else:
          op.add_ok(vm, reason=op.prep_msg(op.SUCCESS_REASON, disk_name=disk.name))
      else:
        op.add_skipped(
          vm,
          reason=('Disk-Type {} is not supported with this gcpdiag runbook, disk name - {}').format(
            disk.type, disk.name
          ),
        )
...
class DiskIopsThroughputUtilisationChecks(runbook.Step):
  ...
  def execute(self):
    ...
    if hasattr(self, 'start_time'):
      ...
      self.start_formatted_string = op.get(flags.START_TIME).strftime('%Y/%m/%d %H:%M:%S')

    if hasattr(self, 'end_time'):
      self.end_formatted_string = self.end_time
    else:
      self.end_formatted_string = op.get(flags.END_TIME).strftime('%Y/%m/%d %H:%M:%S')
    ...
    if cpu_count_query:
      ...
      op.add_failed(
        ...
        remediation=('\tPlease first start the VM {}, if it is not in running state').format(
          vm.short_path
        ),
      )
      return
    ...
    total_disk_size = {'pd-balanced': 0, 'pd-ssd': 0, 'pd-standard': 0, 'pd-extreme': 0}
    provisions_iops = {'pd-balanced': 0, 'pd-ssd': 0, 'pd-standard': 0, 'pd-extreme': 0}
    disk: gce.Disk
    ...
    for disktypes in total_disk_size.items():
      ...
      if total_disk_size[disktype] > 0 and cpu_count > 0:
        if vm_family in ['a', 'f', 'g', 'm']:
          if vm.machine_type().split('-')[0].upper() in ['A2', 'A3', 'F1', 'G2', 'M1', 'M2', 'M3']:
            ...
            if disktype == 'pd-standard' and int(total_disk_size[disktype]) < 100:
              op.info(
                (
                ...
                ).format(disktype, int(total_disk_size[disktype]), 75, 12, 150, 12)
              )

              self.actual_usage_comparision(
                vm, disktype, 75 * disk_io_util_threshold, 'max_read_ops_count'
              )
              self.actual_usage_comparision(
                vm, disktype, 12 * disk_io_util_threshold * 1000 * 1000, 'max_read_bytes_count'
              )
              self.actual_usage_comparision(
                vm, disktype, 150 * disk_io_util_threshold, 'max_write_ops_count'
              )
              self.actual_usage_comparision(
                vm, disktype, 12 * disk_io_util_threshold * 1000 * 1000, 'max_write_bytes_count'
              )

            elif disktype == 'pd-extreme' and vm.machine_type() in ['g1-small', 'f1-micro']:
              op.add_skipped(
                vm,
                reason=(
                  'The script do not support pd-extreme disk type with machine type {} \n'
                ).format(next_hop_val),
              )
            else:
              op.info(
              ...
              self.actual_usage_comparision(
                vm, disktype, min(data[0], data[1]) * disk_io_util_threshold, 'max_read_ops_count'
              )
              self.actual_usage_comparision(
                vm,
                disktype,
              ...
              self.actual_usage_comparision(
                vm, disktype, min(data[4], data[5]) * disk_io_util_threshold, 'max_write_ops_count'
              )
              self.actual_usage_comparision(
                vm,
                disktype,
            ...
            op.add_failed(
              vm,
              reason=('The machine type {} is not supported with this gcpdiag runbook').format(
                vm.machine_type()
              ),
              remediation='You may only run this runbook for any of the below machine family:'
              'A2, A3, C2, C2D, C3, C3D, E2, F1, G1, G2,'
              ' H3, N1, N2, N2D, M1, M2, M3, T2D, T2A, Z3',
            )

        elif vm_family in ['c', 'h', 'z']:
          if vm.machine_type().split('-')[0].upper() in ['C2', 'C2D', 'C3', 'C3D', 'H3', 'Z3']:
            ...
            if disktype == 'pd-standard' and int(total_disk_size[disktype]) < 100:
              ...
              self.actual_usage_comparision(
                vm,
                disktype,
                min(data[1], provisions_iops['pd-extreme']) * disk_io_util_threshold,
                'max_read_ops_count',
              )
              self.actual_usage_comparision(
                vm,
          ...
          if vm.machine_type().split('-')[0].upper() in ['T2D', 'T2A']:
            ...
            elif disktype == 'pd-standard' and int(total_disk_size[disktype]) < 100:
              # upto first 100GB, pd-standard disks have fixed IO limits
              op.info(
                (
                  '\n\tIOPS and Throughput limits available for VM DiskType - {},'
          ...
          if vm.machine_type().split('-')[0].upper() in ['E2']:
            ...
            if disktype == 'pd-extreme':
              ...
              self.actual_usage_comparision(
                vm,
                disktype,
                min(data[5], provisions_iops['pd-extreme']) * disk_io_util_threshold,
                'max_write_ops_count',
              )
              self.actual_usage_comparision(
                vm,
          ...
          if vm.machine_type().split('-')[0].upper() in ['N1', 'N2', 'N2D', 'CUSTOM']:
            next_hop = 'VM vCPU count'
            if vm.machine_type().split('-')[0] == 'custom':
              search_str = 'N1 VMs'
            else:
            ...
            if disktype in ['pd-balanced', 'pd-ssd']:
              ...
              if search_str == 'N2 VMs' and cpu_count > 63:
                ...
                self.actual_usage_comparision(
                  vm,
                  disktype,
                  max(data[2], data[3]) * disk_io_util_threshold * 1000 * 1000,
                  'max_read_bytes_count',
                )
                self.actual_usage_comparision(
                  vm,
                ...
                self.actual_usage_comparision(
                  vm,
                  disktype,
                  max(data[6], data[7]) * disk_io_util_threshold * 1000 * 1000,
                  'max_write_bytes_count',
                )
          else:
            op.add_failed(
  ...
  def limit_calculator(
    ...
    r_iops_limit_per_gb = float(limits_data['iops'][disktype]['Read IOPS per GiB'])
    w_iops_limit_per_gb = float(limits_data['iops'][disktype]['Write IOPS per GiB'])
    ...
    baseline_iops = float(limits_data['baseline'][disktype]['Baseline IOPS per VM'])
    baseline_throughput = float(
      limits_data['baseline'][disktype]['Baseline Throughput (MiBps) per VM']
    )

    # Calculating VM Baseline performance
    vm_baseline_performance_r_iops = baseline_iops + (r_iops_limit_per_gb * total_disk_size)
    vm_baseline_performance_w_iops = baseline_iops + (w_iops_limit_per_gb * total_disk_size)
    ...
    return (
      vm_baseline_performance_r_iops,
      max_read_iops,
      vm_baseline_performance_r_throughput,
      max_read_throuput,
      vm_baseline_performance_w_iops,
      max_write_iops,
      vm_baseline_performance_w_throughput,
      max_write_throuput,
    )

  def actual_usage_comparision(
    self, vm: gce.Instance, disktype: str, utilization_threshold: float, metric_name: str
  ):
    ...
    try:
      ...
      comp = monitoring.query(
        ...
        """
                    ...
                    | {}
                    """.format(
          metric_name, vm.id, storage_type, metric_name_value, utilization_threshold, mon_within_str
        ),
      )
    except googleapiclient.errors.HttpError:
      op.add_skipped(
        project,
        reason=('Not able to fetch monitoring metric {} data for VM {} - Disktype - {}').format(
          metric_name, op.get(flags.INSTANCE_NAME), disktype
        ),
      )
    else:
      if comp:
