...
class TestEnsureInstanceResolved(unittest.TestCase):
  ...
  def test_instance_already_resolved(self):
    self.mock_get.side_effect = {flags.INSTANCE_ID: '123', flags.INSTANCE_NAME: 'test-instance'}.get
  ...
  def test_instance_resolved_by_name(self):
    ...
    util.ensure_instance_resolved()
    self.mock_get_instance.assert_called_with(
      project_id='test-project', zone='us-central1-a', instance_name='test-instance'
    )
  ...
  def test_instance_resolved_by_id(self):
    ...
    util.ensure_instance_resolved()
    self.mock_get_instance.assert_called_with(
      project_id='test-project', zone='us-central1-a', instance_name='123'
    )
    self.mock_put.assert_any_call(flags.INSTANCE_NAME, 'test-instance')
    self.mock_put.assert_any_call(flags.INSTANCE_ID, '123')
...
class TestUtil:
  ...
  def test_has_no_valid_key(self):
    local_user = 'notvalid'
    assert not util.user_has_valid_ssh_key(local_user=local_user, keys=self.keys)
  ...
  def test_has_at_least_one_valid_key_and_type(self):
    local_user = 'test'
    key_type = 'ssh-rsa'
    assert util.user_has_valid_ssh_key(local_user=local_user, keys=self.keys, key_type=key_type)

  def test_has_at_least_one_valid_key_and_mismatched_type(self):
    local_user = 'test'
    key_type = 'ssh-dss'
    assert not util.user_has_valid_ssh_key(local_user=local_user, keys=self.keys, key_type=key_type)
