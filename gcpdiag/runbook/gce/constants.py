...
CLOUD_INIT_STARTUP_PATTERN = [r"cloud-init\[(\d+)\]: Cloud-init v\. (.*?) running '(.*)'"]
...
GCE_ACTIVITY_LOG_FILTER = (
  'logName:"cloudaudit.googleapis.com%2Factivity" protoPayload.serviceName:"compute.googleapis.com"'
)
...
RESERVATION_INVALID_SOURCE_RESOURCE = ['Source resource reference provided invalid.']
RESERVATION_ALREADY_EXISTS = [r'The resource \'projects/.+/zones/.+/.+/.+\' already exists']
...
RESERVATION_SHARED_OWNER_PROJECTS_CONSTRAINT = [
  'Constraint constraints/compute.sharedReservationsOwnerProjects violated for project'
]
...
RESERVATION_CREATE_SUCCESS_FILTER = (
  "protoPayload.methodName='compute.reservations.create' AND protoPayload.status.code=200"
)
RESERVATION_UPDATE_SUCCESS_FILTER = (
  "protoPayload.methodName='compute.reservations.update' AND protoPayload.status.code=200"
)
