...
class InvestigateVmCreationLogFailure(runbook.Gateway):
  ...
  def execute(self):
    ...
    if res:
      entry = res[0]
      error_message = get_path(entry, ('protoPayload', 'response', 'error', 'errors', 0, 'message'))
      error_reason = get_path(entry, ('protoPayload', 'response', 'error', 'errors', 0, 'reason'))
      ...
      if status_message and status_message == 'QUOTA_EXCEEDED':
        ...
        metric_name = get_path(
          entry, ('protoPayload', 'status', 'details', 'quotaExceeded', 'metricName')
        )
        limit = get_path(entry, ('protoPayload', 'status', 'details', 'quotaExceeded', 'limit'))
        limit_name = get_path(
          entry, ('protoPayload', 'status', 'details', 'quotaExceeded', 'limitName')
        )

        op.add_failed(
          project,
          reason=op.prep_msg(op.FAILURE_REASON_ALT1, error_message=error_message),
          remediation=op.prep_msg(
            op.FAILURE_REMEDIATION_ALT1, metric_name=metric_name, limit=limit, limit_name=limit_name
          ),
        )
      elif error_reason and error_reason == 'alreadyExists':
        op.add_failed(
          project,
          reason=op.prep_msg(op.FAILURE_REASON, error_message=error_message),
          remediation=op.prep_msg(
        ...
        op.add_failed(
          project,
          reason=op.prep_msg(op.FAILURE_REASON_ALT2, error_message=error_message),
