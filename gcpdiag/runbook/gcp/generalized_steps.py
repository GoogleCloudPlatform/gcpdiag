...
class HumanTask(runbook.Step):
  ...
  def execute(self):
    ...
    if instructions:
      response = op.prompt(kind=op.CONFIRMATION, message=instructions, options=options)
      if response == op.UNCERTAIN:
        response = op.add_uncertain(
...
class ResourceAttributeCheck(runbook.Step):
  ...
  def execute(self):
    ...
    if self.extract_args is not None:
      for k, v in self.extract_args.items():
        if v['source'] == Resource:
          self.message_args[k] = get_path(
            self.resource._resource_data, v['attribute'], default=v.get('default')
          )
    ...
    if hasattr(self, 'attribute'):
      actual_value = get_path(self.resource._resource_data, self.attribute, default=None)
      if self.expected_value == actual_value:
        op.add_ok(self.resource, op.prep_msg(op.SUCCESS_REASON, **self.message_args))
      else:
        op.add_failed(
          self.resource,
          reason=op.prep_msg(op.FAILURE_REASON, **self.message_args),
          remediation=op.prep_msg(op.FAILURE_REMEDIATION, **self.message_args),
        )
  ...
  def execute(self):
    ...
    if self.expected_state == actual_state:
      op.add_ok(
        project,
        op.prep_msg(
          op.SUCCESS_REASON, service_name=service_name, expected_state=self.expected_state.value
        ),
      )
    else:
      remediation = ''
      if self.expected_state == constants.APIState.ENABLED:
        remediation = op.prep_msg(
          op.FAILURE_REMEDIATION, service_name=service_name, project_id=op.get(flags.PROJECT_ID)
        )
      ...
      op.add_failed(
        project,
        reason=op.prep_msg(
          op.FAILURE_REASON, service_name=service_name, expected_state=self.expected_state.value
        ),
