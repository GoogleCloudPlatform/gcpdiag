...
"""
This runbook module checks for issues related to service accounts, which is a
common source of failures for Dataproc clusters.
"""
...
class ServiceAccountRule(runbook.BaseRule):
  ...
  def run(self, context: runbook.RunbookContext):
    ...
    cluster = dataproc.get_cluster(context.project_id, context.region,
                                   context.cluster_name)
    # This is a placeholder for the actual service account check logic.
    ...
    runbook.add_ok_rule(
        report=runbook.Report(
            short_desc='No service account issues were found.',
            long_desc=
            'This is a placeholder for the actual service account check logic.'
        ))
  ...
  def solution(self):
    return (
        '**Service accounts** are special Google accounts that can be used by applications to make authorized API calls.'
    )
