...
from gcpdiag.queries import crm, dataproc, gce, iam, logs, network, networkmanagement
...
class ClusterCreation(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    flags.CONSTRAINT: {
      'type': bool,
      'help': ('Checks if the Dataproc cluster has an enforced organization policy constraint'),
    },
    flags.STACKDRIVER: {
      'type': str,
    ...
    flags.INTERNAL_IP_ONLY: {
      'type': bool,
      'help': ('Checks if the Dataproc cluster has been created with only Internal IP'),
    },
    flags.START_TIME: {
      'type': datetime,
  ...
  def legacy_parameter_handler(self, parameters):
    ...
    if flags.CLUSTER_NAME in parameters:
      parameters[flags.DATAPROC_CLUSTER_NAME] = parameters.pop(flags.CLUSTER_NAME)
      ...
      if cluster:
        ...
        if cluster.status == 'ERROR':
          ...
          if network.get_network_from_url(op.get(flags.DATAPROC_NETWORK)):
            op.put(
              flags.HOST_VPC_PROJECT_ID,
              network.get_network_from_url(op.get(flags.DATAPROC_NETWORK)).project_id,
            )
        else:
          op.add_skipped(
            project,
  ...
  def execute(self):
    ...
    if cluster is None:
      ...
      if cluster_zone is None:
        op.add_skipped(
          cluster,
          reason=(f'Zone cannot be retrieved from the cluster. Zone: {cluster_zone}'),
        )
        return
      # Skip DPGKE clusters
      ...
      if cluster.is_single_node_cluster:
        op.add_skipped(cluster, reason='This is a single node cluster, skipping this step.')
      ...
      target_ip = target.get_network_ip_for_instance_interface(cluster.gce_network_uri)
      ...
      source_ip = source.get_network_ip_for_instance_interface(cluster.gce_network_uri)
      ...
      op.info(
        'Connectivity test result: ' + connectivity_test_result['reachabilityDetails']['result']
      )
      if connectivity_test_result['reachabilityDetails']['result'] != 'REACHABLE':
        is_connectivity_fine = False
        for trace in connectivity_test_result['reachabilityDetails']['traces']:
          op.info('Endpoint details: ' + json.dumps(trace['endpointInfo'], indent=2))
          last_step = None
        ...
        op.info(
          'ICMP traffic must be allowed. Check the result of the'
          ' connectivity '
      ...
      if is_connectivity_fine:
        op.add_ok(cluster, op.prep_msg(op.SUCCESS_REASON, cluster_name=cluster.name))
      else:
        op.add_failed(
          cluster,
...
class CheckSharedVPCRoles(runbook.Step):
  ...
  def execute(self):
    ...
    project = crm.get_project(op.get(flags.PROJECT_ID))
    if op.get(flags.HOST_VPC_PROJECT_ID) and (
      op.get(flags.HOST_VPC_PROJECT_ID) != op.get(flags.PROJECT_ID)
    ):
      # Check Service Agent Service Account role:
      service_agent_sa = f'service-{project.number}@dataproc-accounts.iam.gserviceaccount.com'
      service_agent_vpc_permission_check = iam_gs.IamPolicyCheck()
      service_agent_vpc_permission_check.project = op.get(flags.HOST_VPC_PROJECT_ID)
      ...
      op.add_skipped(project, reason='Cluster is not using a Shared VPC network')
