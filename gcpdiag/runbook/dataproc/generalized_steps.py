  ...
  def execute(self):
    ...
    if op.get(flags.JOB_OLDER_THAN_30_DAYS):
      op.add_skipped(
        project,
        reason=(
          'Job is older than 30 days, skipping this step. '
    ...
    if log_entries:
      op.add_failed(
        project,
        reason=op.prep_msg(op.FAILURE_REASON, log=log_message, cluster_name=cluster_name),
...
class CheckClusterNetworkConnectivity(runbook.Step):
  ...
  def execute(self):
    ...
    cluster = dataproc.get_cluster(
      cluster_name=cluster_name, region=op.get(flags.REGION), project=project_id
    )
