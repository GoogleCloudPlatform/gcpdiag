  ...
  rule_parameters = [
    {
      ...
      'service_account': (f'saworker@{DUMMY_PROJECT_ID}.iam.gserviceaccount.com'),
      'region': 'us-central1',
...
class ClusterCreationBuildTreeTest(unittest.TestCase):
  @mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ClusterCreation.add_step')
  @mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ClusterCreation.add_start')
  @mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ClusterCreation.add_end')
  ...
  def test_build_tree(self, mock_op_get, mock_add_end, mock_add_start, mock_add_step):
    ...
    self.assertIsInstance(mock_add_start.call_args[0][0], cluster_creation.ClusterCreationStart)
    ...
    self.assertIsInstance(mock_add_end.call_args[0][0], cluster_creation.ClusterCreationEnd)
...
class ClusterCreationStepTestBase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.params = {
      ...
      flags.DATAPROC_CLUSTER_NAME: 'test-cluster',
      flags.SERVICE_ACCOUNT: None,
      flags.DATAPROC_NETWORK: None,
      flags.ZONE: None,
      flags.INTERNAL_IP_ONLY: None,
      flags.SUBNETWORK: None,
      ...
      flags.HOST_VPC_PROJECT_ID: None,
    }
    ...
    self.mock_dataproc_get_cluster = self.enterContext(
      mock.patch('gcpdiag.queries.dataproc.get_cluster', wraps=dataproc.get_cluster)
    )
...
class ClusterCreationStockoutTest(ClusterCreationStepTestBase):
  def setUp(self):
    ...
    self.add_child_patch = self.enterContext(
      mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ClusterCreationStockout.add_child')
    )
  ...
  def test_add_child_called(self):
    ...
    self.assertIsInstance(self.add_child_patch.call_args[1]['child'], logs_gs.CheckIssueLogEntry)
...
class ClusterCreationQuotaTest(ClusterCreationStepTestBase):
  def setUp(self):
    super().setUp()
    self.add_child_patch = self.enterContext(
      mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ClusterCreationQuota.add_child')
    )
...
class ClusterDetailsDependencyGatewayTest(ClusterCreationStepTestBase):
  ...
  def test_cluster_exists(self):
    ...
    self.assertTrue(
      any(isinstance(s, cluster_creation.CheckInitScriptFailure) for s in steps_added)
    )
    self.assertTrue(any(isinstance(s, cluster_creation.CheckClusterNetwork) for s in steps_added))
    self.assertTrue(any(isinstance(s, cluster_creation.InternalIpGateway) for s in steps_added))
    self.assertTrue(any(isinstance(s, cluster_creation.ServiceAccountExists) for s in steps_added))
    self.assertTrue(any(isinstance(s, cluster_creation.CheckSharedVPCRoles) for s in steps_added))
  ...
  def test_cluster_does_not_exist(self):
    ...
    with op.operator_context(self.operator):
      ...
      step.execute()
    steps_added = [call[0][0] for call in self.add_child_patch.call_args_list]
    self.assertTrue(any(isinstance(s, cluster_creation.ClusterCreationQuota) for s in steps_added))
    self.assertTrue(
      any(isinstance(s, cluster_creation.ClusterCreationStockout) for s in steps_added)
    )
...
class CheckClusterNetworkTest(ClusterCreationStepTestBase):
  def setUp(self):
    ...
    self.mock_instance.get_network_ip_for_instance_interface.return_value = '10.0.0.1/32'
    self.mock_gce_get_instance.return_value = self.mock_instance
    self.mock_networkmanagement_run_connectivity_test.return_value = {
      'reachabilityDetails': {'result': 'REACHABLE', 'traces': []}
    }
  ...
  def test_connectivity_ok(self):
    ...
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
    self.assertEqual(self.mock_networkmanagement_run_connectivity_test.call_count, 3)
    ...
    self.mock_networkmanagement_run_connectivity_test.side_effect = [
      {
        'reachabilityDetails': {
          'result': 'UNREACHABLE',
          'traces': [{'steps': [{'foo': 'bar'}], 'endpointInfo': {}}],
        }
      },
      {'reachabilityDetails': {'result': 'REACHABLE', 'traces': []}},
      {'reachabilityDetails': {'result': 'REACHABLE', 'traces': []}},
...
class InternalIpGatewayTest(ClusterCreationStepTestBase):
  def setUp(self):
    super().setUp()
    self.add_child_patch = self.enterContext(
      mock.patch('gcpdiag.runbook.dataproc.cluster_creation.InternalIpGateway.add_child')
    )
...
class ServiceAccountExistsTest(ClusterCreationStepTestBase):
  def setUp(self):
    ...
    self.add_child_patch = self.enterContext(
      mock.patch('gcpdiag.runbook.dataproc.cluster_creation.ServiceAccountExists.add_child')
    )
  ...
  def test_sa_exists_cross_project(self):
    ...
    self.assertTrue(any(isinstance(s, crm_gs.OrgPolicyCheck) for s in steps_added))
    self.assertEqual(sum(1 for s in steps_added if isinstance(s, iam_gs.IamPolicyCheck)), 3)

  def test_sa_not_exists_cross_project(self):
    self.params[flags.CROSS_PROJECT_ID] = 'gcpdiag-iam1-aaaa'
    self.params[flags.SERVICE_ACCOUNT] = 'non-existent@gcpdiag-iam1-aaaa.iam.gserviceaccount.com'
...
class CheckSharedVPCRolesTest(ClusterCreationStepTestBase):
  def setUp(self):
    super().setUp()
    self.add_child_patch = self.enterContext(
      mock.patch('gcpdiag.runbook.dataproc.cluster_creation.CheckSharedVPCRoles.add_child')
    )
  ...
  def test_shared_vpc(self):
    ...
    with op.operator_context(self.operator):
      self.operator.set_step(step)
      step.execute()
    steps_added = [call[1]['child'] for call in self.add_child_patch.call_args_list]
    self.assertEqual(sum(1 for s in steps_added if isinstance(s, iam_gs.IamPolicyCheck)), 2)
...
class ClusterCreationEndTest(ClusterCreationStepTestBase):
  ...
  def test_cluster_does_not_exist(self):
    ...
    with op.operator_context(self.operator):
      ...
      step.execute()
    self.mock_interface.info.assert_called_once()
    self.assertIn('Some steps were skipped', self.mock_interface.info.call_args[0][0])
