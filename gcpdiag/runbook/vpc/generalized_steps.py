...
class VpcRouteCheck(runbook.Step):
  ...
  def execute(self):
    ...
    nic_info = util.get_nic_info(vm.get_network_interfaces, op.get(flags.SRC_NIC))

    project, netwrk, dest_ip = nic_info['project'], nic_info['network'], op.get(flags.DEST_IP)
    # get the selected route for the destination
    selected_route = util.get_selected_route_for_dest_ip(project, netwrk, dest_ip)
    ...
    if selected_route:
      ...
      if next_hop['type'] == 'nextHopGateway':
        op.add_ok(vm, op.prep_msg(op.SUCCESS_REASON, route_name=selected_route.name))
      else:
      ...
      op.add_failed(
        vm,
        reason=op.prep_msg(op.FAILURE_REASON, address=op.get(flags.DEST_IP)),
        remediation=op.prep_msg(op.FAILURE_REMEDIATION),
      )
...
class VmExternalIpConnectivityTest(runbook.Step):
  ...
  def execute(self):
    ...
    result = networkmanagement.run_connectivity_test(
      ...
      op.get(flags.PROTOCOL_TYPE),
    )
    # if the VM has only a private IP address, check that there is a NATGW in
    # the traffic path and put the NATGW details
    if result and not util.is_external_ip_on_nic(vm.get_network_interfaces, op.get(flags.SRC_NIC)):
      steps = result['reachabilityDetails']['traces'][0]['steps']
      if 'CLOUD_NAT' in str(steps):
        natgw_step = [step for step in steps if step.get('nat', {}).get('type', {}) == 'CLOUD_NAT']
