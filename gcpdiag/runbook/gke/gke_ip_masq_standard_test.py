...
class TestGkeIpMasqStandard(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.enterContext(
      mock.patch('gcpdiag.runbook.op.prep_msg', side_effect=lambda x, **y: x, autospec=True)
    )
  ...
  def test_execute_found_cluster_with_name_and_location(
    self, mock_add_ok, mock_op_get, mock_get_clusters, mock_get_project
  ):
  ...
  def test_execute_uncertain_when_only_location_provided(
    self, mock_add_uncertain, mock_op_get, mock_get_clusters, mock_get_project
  ):
  ...
  def test_build_tree_constructs_full_diagnostic_path(
    self, mock_add_end, mock_add_start, mock_add_step
  ):
  ...
  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.runbook.op.add_ok')
  def test_nodeproblem_success_on_logs_found(
    self, mock_add_ok, mock_op_get, mock_realtime_query, mock_get_project
  ):
...
class TestGkeIpMasqStandardCoverage(unittest.TestCase):
  ...
  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.runbook.op.add_ok')
  def test_execute_found_cluster_by_name_only(
    self, mock_add_ok, mock_op_get, mock_get_clusters, mock_get_project
  ):
  ...
  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.runbook.op.add_skipped')
  def test_execute_skipped_when_cluster_name_not_found(
    self, mock_add_skipped, mock_op_get, mock_get_clusters, mock_get_project
  ):
  ...
  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.runbook.op.add_skipped')
  def test_execute_skipped_when_no_clusters_at_location(
    self, mock_add_skipped, mock_op_get, mock_get_clusters, mock_get_project
  ):
  ...
  def test_nodeproblem_failed_on_no_logs(
    self, mock_add_failed, mock_op_get, mock_realtime_query, mock_get_project
  ):
  ...
  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.runbook.op.add_uncertain')
  def test_diagnostic_steps_uncertain_output(
    self, mock_add_uncertain, mock_op_get, mock_get_project
  ):
