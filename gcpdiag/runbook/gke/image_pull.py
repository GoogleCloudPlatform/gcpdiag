...
class ImagePull(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    flags.START_TIME: {
      'type': datetime,
      'help': '(Optional) The start window to query the logs. Format: YYYY-MM-DDTHH:MM:SSZ',
      'required': False,
    },
    flags.END_TIME: {
      'type': datetime,
      'help': '(Optional) The end window for the logs. Format: YYYY-MM-DDTHH:MM:SSZ',
      'required': False,
    },
  }
  ...
  def build_tree(self):
    ...
    self.add_step(parent=image_dns_issue, child=image_connection_timeout_restricted_private)
    self.add_step(
      parent=image_connection_timeout_restricted_private, child=image_connection_timeout
    )
    self.add_step(parent=image_connection_timeout, child=image_not_found_insufficient_scope)
    # Ending runbook
...
class ImageNotFound(runbook.Step):
  ...
  def execute(self):
    ...
    if log_entries:
      ...
      op.add_ok(
        project_path,
        reason=op.prep_msg(op.SUCCESS_REASON, start_time=start_time, end_time=end_time),
      )
...
class ImagePullEnd(runbook.EndStep):
  ...
  def execute(self):
    ...
    response = op.prompt(
      kind=op.CONFIRMATION, message='Are you satisfied with the `GKE Image Pull runbbok` analysis?'
    )
    if response == op.NO:
...
def format_log_entries(log_entries):
  ...
  labels = get_path(log_entry, ('resource', 'labels'), default={})  # Provide default empty dict
  if labels:
  ...
  json_payload = get_path(log_entry, ('jsonPayload',), default={})  # Provide default empty dict
  formatted_log.extend(
    [
