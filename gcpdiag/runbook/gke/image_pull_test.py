...
class TestImagePull(unittest.TestCase):
  ...
  def test_image_pull_start_skips_when_logging_disabled(
    self, mock_add_skipped, mock_get_project, mock_is_enabled, mock_op_get
  ):
    """ImagePullStart should skip execution if the Logging API is disabled."""
    del mock_get_project
    mock_op_get.side_effect = lambda key: self.project_id if key == flags.PROJECT_ID else None
  ...
  @mock.patch('gcpdiag.runbook.gke.image_pull.crm.get_project')
  @mock.patch('gcpdiag.runbook.gke.image_pull.op.get')
  def test_image_not_found_fails_when_logs_detected(
    self, mock_op_get, mock_get_project, mock_add_failed, mock_query
  ):
    ...
    mock_query.return_value = [
      {
        'resource': {'labels': {'pod_name': 'test-pod'}},
  ...
  def test_image_pull_end_shows_info_on_no_response(self, mock_op_info, mock_op_prompt):
...
class TestImagePullCoverage(unittest.TestCase):
  ...
  @mock.patch('gcpdiag.runbook.gke.image_pull.op.get')
  def test_local_realtime_query_correctly_calls_logs_query(self, mock_op_get, mock_logs_query):
    ...
    mock_logs_query.assert_called_once_with(
  ...
  @mock.patch('gcpdiag.runbook.gke.image_pull.crm.get_project')
  @mock.patch('gcpdiag.runbook.gke.image_pull.op.get')
  def test_steps_fail_on_matching_log_entries(
    self, mock_op_get, mock_get_project, mock_add_failed, mock_query
  ):
    """Verify Failure paths for specific error classes like Forbidden or DNS issues."""
    del mock_get_project
    mock_op_get.return_value = 'test'
    mock_query.return_value = [{'jsonPayload': {'message': 'error message'}, 'resource': {}}]
    steps = [
      image_pull.ImageForbidden(),
      image_pull.ImageDnsIssue(),
