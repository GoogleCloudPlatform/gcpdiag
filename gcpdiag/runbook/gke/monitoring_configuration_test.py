...
class TestMonitoringConfiguration(unittest.TestCase):
  ...
  def test_start_step_no_clusters(self, mock_skipped, mock_crm, mock_gke, mock_get):
  ...
  def test_cluster_monitoring_entirely_disabled(self, mock_failed, mock_util, mock_gke, mock_get):
    ...
    self.assertIn('Monitoring entirely disabled', mock_failed.call_args[1]['remediation'])

  @mock.patch('gcpdiag.runbook.op.get')
  @mock.patch('gcpdiag.queries.gke.get_clusters')
  @mock.patch('gcpdiag.lint.gke.util.get_cluster_object')
  @mock.patch('gcpdiag.runbook.op.add_failed')
  def test_cluster_monitoring_missing_gpu_metrics(self, mock_failed, mock_util, mock_gke, mock_get):
    ...
    gpu_taint = {'key': 'nvidia.com/gpu', 'value': 'present', 'effect': 'NO_SCHEDULE'}
  ...
  def test_cluster_monitoring_autopilot_skipped(self, mock_ok, mock_util, mock_gke, mock_get):
...
class TestMonitoringConfigurationCoverage(unittest.TestCase):
  ...
  @mock.patch('gcpdiag.runbook.op.add_skipped')
  @mock.patch('gcpdiag.runbook.op.get')
  def test_start_execute_no_clusters(
    self, mock_op_get, mock_add_skipped, mock_get_clusters, mock_get_project
  ):
    ...
    with op.operator_context(mock.Mock(parameters={'project_id': 'test-project'})):
      step.execute()

    mock_add_skipped.assert_called_once()
    self.assertIn('No GKE clusters found', mock_add_skipped.call_args[1]['reason'])

  @mock.patch('gcpdiag.queries.crm.get_project')
  @mock.patch('gcpdiag.queries.gke.get_clusters')
  @mock.patch('gcpdiag.runbook.op.get')
  def test_start_execute_with_parameters(self, mock_op_get, mock_get_clusters, mock_get_project):
    ...
    mock_get_clusters.return_value = {'c1': MockCluster('cluster-1', 'us-central1')}
    step = monitoring_configuration.MonitoringConfigurationStart()
  ...
  @mock.patch('gcpdiag.lint.gke.util.get_cluster_object')
  @mock.patch('gcpdiag.runbook.op.add_ok')
  def test_cluster_level_monitoring_standard_path(
    self, mock_add_ok, mock_get_cluster_obj, mock_op_get, mock_get_clusters
  ):
    ...
    cluster = MockCluster('cluster-1', 'us-central1', is_autopilot=False, metrics=metrics)
    mock_get_cluster_obj.return_value = cluster
    step = monitoring_configuration.ClusterLevelMonitoringConfigurationEnabled()

    with op.operator_context(
      mock.Mock(
        parameters={'project_id': 'p', 'location': 'us-central1', 'gke_cluster_name': 'cluster-1'}
      )
    ):
      step.execute()
