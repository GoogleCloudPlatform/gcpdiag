...
from gcpdiag.runbook import Bundle, DiagnosticEngine, Step, constants, exceptions
...
class LegacyParamHandler(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    'deprecated_param': {'new_parameter': 'new_param', 'deprecated': True},
  }
...
class TestDiagnosticEngine(unittest.TestCase):
  ...
  def test_run_diagnostic_tree_missing_required_parameters(self):
    with self.assertRaises(exceptions.MissingParameterError) as cm:
      ...
      dt.parameters = {
        'missing_param': {'required': True, 'help': 'Missing parameter help'},
      }
      self.de._check_required_paramaters(
        parameter_def=dt.parameters, caller_args=models.Parameter()
      )
    ...
    self.assertIn('Parameter Explanation: Missing parameter help', str(cm.exception))
  ...
  def test_run_diagnostic_tree_deprecated_parameters(self, mock_logging_error):
    parameters = {'deprecated_param': {'deprecated': True, 'new_parameter': 'new_value'}}
    result = self.de._check_deprecated_paramaters(
      parameter_def=parameters, caller_args=models.Parameter({'deprecated_param': 'val'})
    )
  ...
  def test_both_new_and_deprecated_missing(self):
    with self.assertRaises(exceptions.MissingParameterError):
      ...
      self.de._check_required_paramaters(parameter_def=dt.parameters, caller_args=user_supplied)
  ...
  def test_backward_compatibility_for_deprecated_params(self):
    ...
    self.de._check_required_paramaters(parameter_def=dt.parameters, caller_args=user_supplied_param)
  ...
  def test_find_path_dfs_normal_operation(self, mock_run_step):
    ...
    self.de.find_path_dfs(operator=op, step=current_step, executed_steps=visited)

    mock_run_step.assert_called()
    self.assertIn(current_step, visited)  # Check if the current step was marked as visited
  ...
  def test_find_path_dfs_dynamic_end_step_stops_execution(self, mock_run_step):
    ...
    finalize_step = Mock(execution_id='finalize_investigation', type=constants.StepType.END)
    ...
    def side_effect_for_run_step(step, operator):
  ...
  def test_gateway_step_in_bundle(self, mock_run_step, mock_operator_class):
    ...
    class MockChildStep(runbook.Step):
      ...
      def execute(self, op):
        ...
        mock_child_execute()
        op.add_ok(resource=Mock(spec=models.Resource), reason='Child step executed')
    ...
    mock_op_instance.add_ok.assert_called_once_with(resource=ANY, reason='Child step executed')
  ...
  def test_run_bundles(self, mock_run_step):
    bundle = runbook.Bundle()
    param = {'param_one': 'test', 'param_two': True, 'param_three': datetime.now()}
  ...
  @patch('gcpdiag.runbook.DiagnosticEngine.run_step')
  def test_run_multiple_bundles_consolidated_report(self, mock_run_step):
...
class TestSetDefaultParameters(unittest.TestCase):
  ...
  def test_no_parameters_set(self):
    ...
    self.assertAlmostEqual(end_time - start_time, timedelta(hours=8), delta=timedelta(seconds=10))
  ...
  def test_end_time_provided_in_rfc3339(self):
    ...
    exp_end_time = datetime.strptime(end_t_str, '%Y-%m-%dT%H:%M:%SZ').replace(tzinfo=timezone.utc)
  ...
  def test_only_start_time_provided_in_rfc3339(self):
    ...
    exp_start_time = datetime.strptime(start_t_str, '%Y-%m-%dT%H:%M:%SZ').replace(
      tzinfo=timezone.utc
    )
  ...
  def test_both_times_provided_in_rfc3339(self):
    ...
    expected_start_time = datetime.strptime(start_time_str, '%Y-%m-%dT%H:%M:%SZ').replace(
      tzinfo=timezone.utc
    )
    expected_end_time = datetime.strptime(end_time_str, '%Y-%m-%dT%H:%M:%SZ').replace(
      tzinfo=timezone.utc
    )
  ...
  def test_times_provided_in_epoch_format(self):
    ...
    expected_start_time = datetime.fromtimestamp(float(start_time_epoch), tz=timezone.utc)
    expected_end_time = datetime.fromtimestamp(float(end_time_epoch), tz=timezone.utc)
    self.assertEqual(start_time, expected_start_time)
    self.assertEqual(end_time, expected_end_time)
