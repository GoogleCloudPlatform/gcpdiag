...
class UnhealthyBackends(runbook.DiagnosticTree):
  ...
  parameters = {
    ...
    flags.BACKEND_SERVICE_NAME: {
      'type': str,
      'help': ('The name of the backend service that you want to investigate'),
      'required': True,
    },
    flags.REGION: {
      'type': str,
  ...
  def build_tree(self):
    ...
    if not apis.is_enabled(project_id, 'compute'):
      start.error_message = 'Compute Engine API is not enabled. Please enable it and try again.'
      return

    # Fetch all primary resources once to avoid redundant API calls
    try:
      backend_service = lb.get_backend_service(project_id, backend_service_name, region)
    ...
    health_check = gce.get_health_check(
      project_id, backend_service.health_check, backend_service.health_check_region
    )
...
class UnhealthyBackendsStart(runbook.StartStep):
  ...
  def name(self):
    return (
      f'Analyze unhealthy backends for backend service'
  ...
  def execute(self):
    ...
    if not self.backend_service:
      op.add_skipped(
        proj,
        reason=(
          f'Backend service {self.backend_service_name} does not'
    ...
    unhealthy_backends = [
      backend for backend in self.backend_health_statuses if backend.health_state == 'UNHEALTHY'
    ]

    backend_health_statuses_per_group = {
      k: list(v) for k, v in groupby(self.backend_health_statuses, key=lambda x: x.group)
    }

    if unhealthy_backends:
      detailed_reason = ''
      for group, backends_in_group in backend_health_statuses_per_group.items():
        unhealthy_count = sum(1 for x in backends_in_group if x.health_state == 'UNHEALTHY')
        detailed_reason += (
          f'Group {group} has {unhealthy_count}/{len(backends_in_group)} unhealthy backends\n'
        )
      op.add_failed(
        ...
        reason=op.prep_msg(
          ...
          timing_and_threshold=_get_timing_and_threshold_info(self.health_check),
        ),
        remediation='',
      )
    else:
      ...
      for group in all_groups:
        ...
        if resource_type == 'instanceGroups':
          ...
          group_metadata[('gce_instance_group', resource_name, location)] = group
        elif resource_type == 'networkEndpointGroups':
          neg = _get_zonal_network_endpoint_group(self.project_id, location, resource_name)
          if neg:
            filter_part = (
            ...
            group_filters.append(filter_part)
            group_metadata[('gce_network_endpoint_group', neg.id, location)] = group
      ...
      if group_filters:
        ...
        aggregated_filter = """log_name="projects/{}/logs/compute.googleapis.com%2Fhealthchecks"
                            ...
                            AND ({})
                            """.format(self.project_id, ' OR '.join(group_filters))
        ...
        if log_entries:
          ...
          for entry in log_entries:
            ...
            name = labels.get('instance_group_name') or labels.get('network_endpoint_group_id')
...
class CheckVmPerformance(runbook.CompositeStep):
  ...
  def name(self):
    return (
      f'Check VMs performance for unhealthy backends in backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    for status in sorted(
      self.backend_health_statuses, key=lambda obj: obj.instance
    ):  # sorting to make testing predictable
    ...
    for group, instance in instances_to_analyze_by_group.items():
      m = re.search(r'projects/([^/?]+)/zones/([^/?]+)/instances/([^/?]+)', instance)
      if not m:
        raise RuntimeError(
          f"Can't determine project, zone or instance name from self links {group}"
        )

      project_id = m.group(1)
      zone = m.group(2)
      instance_name = m.group(3)
      instance_object = gce.get_instance(
        project_id=project_id, zone=zone, instance_name=instance_name
      )
      if not instance_object:
        op.add_skipped(
          None, reason=f'VM instance {instance_name} not found in project {project_id} zone {zone}'
        )
...
class VerifyFirewallRules(runbook.Step):
  ...
  def name(self):
    return (
      f'Verify firewall rules allow health checks for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    if not apis.is_enabled(self.project_id, 'recommender'):
      op.add_skipped(
        ...
        reason=('Checking firewall rules requires Recommender API to be enabled'),
      )
      return
    ...
    for insight in insights:
      if insight.is_firewall_rule_insight and insight.details.get('loadBalancerUri'):
        # network load balancers (backend service is central resource):
        if insight.details.get('loadBalancerUri').endswith(self.backend_service.full_path):
    ...
    op.add_ok(
      ...
      reason=op.prep_msg(op.SUCCESS_REASON, bs_url=self.backend_service.full_path),
    )
...
class ValidateBackendServicePortConfiguration(runbook.Step):
  ...
  def name(self):
    return (
      f'Validate port configuration for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    if not apis.is_enabled(self.project_id, 'recommender'):
      op.add_skipped(
        crm.get_project(self.project_id),
        reason=('Checking port configuration requires Recommender API to be enabled'),
      )
      return
    ...
    for insight in insights:
      if insight.is_health_check_port_mismatch_insight:
        for info in insight.details.get('backendServiceInfos'):
          if info.get('backendServiceUri').endswith(self.backend_service.full_path):
            impacted_igs = [
              igs.get(self._normalize_url(x)) for x in info.get('impactedInstanceGroupUris', [])
            ]
    ...
    has_negs = any(
      '/networkEndpointGroups/' in backend.get('group') for backend in self.backend_service.backends
    )

    if has_negs:
      port_mapping = (
        'Backend service uses Network Endpoint Groups (NEGs), portName parameter is not applicable.'
      )
      op.add_ok(
        self.backend_service,
        ...
        if ig:
          ...
          if port_numbers:
            ...
            port_mapping_details.append(
              f'  {ig.full_path}: portName "{port_name}" -> port(s) {port_numbers_str}'
            )
          else:
            port_mapping_details.append(f'  {ig.full_path}: portName "{port_name}" not defined')

      if port_mapping_details:
        port_mapping = f'  Health check port specification: {self.health_check.port_specification}'
        ...
        port_mapping = (
          f'portName "{port_name}" is not used in any instance group for this backend service.'
        )
  ...
  def _normalize_url(self, url):
    ...
    try:
      ...
      elif parsed_url.hostname == 'www.googleapis.com' and parsed_url.path.startswith(
        '/compute/v1/'
      ):
  ...
  def _format_affected_instance_groups(self, impacted_instance_groups, serving_port_name):
    output_lines = []
    for group in impacted_instance_groups:
      port_numbers = ', '.join(self._get_port_numbers_by_name(group, serving_port_name))
      output_lines.append(
        f'{group.full_path} - port name "{serving_port_name}" translates to port {port_numbers}'
      )

    return '\n'.join(output_lines)

  def _get_port_numbers_by_name(self, impacted_instance_group, serving_port_name):
    return [
      str(p['port'])
...
class ValidateBackendServiceProtocolConfiguration(runbook.Step):
  ...
  def name(self):
    return (
      f'Validate protocol configuration for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
...
class VerifyHealthCheckLoggingEnabled(runbook.Gateway):
  ...
  def name(self):
    return (
      f'Verify health check logging enabled for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    if self.health_check.is_log_enabled:
      op.add_ok(
        self.health_check,
        reason=op.prep_msg(op.SUCCESS_REASON, hc_url=self.health_check.full_path),
      )
      ...
      op.add_uncertain(
        self.backend_service,
        reason=op.prep_msg(op.UNCERTAIN_REASON, hc_url=self.health_check.full_path),
        remediation=op.prep_msg(
          op.UNCERTAIN_REMEDIATION,
          hc_name=self.health_check.name,
...
class AnalyzeLatestHealthCheckLog(runbook.Gateway):
  ...
  def name(self):
    return (
      f'Analyze latest health check log for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    unhealthy_groups = {
      state.group for state in self.backend_health_statuses if state.health_state == 'UNHEALTHY'
    }
    ...
    for group in unhealthy_groups:
      m = re.search(r'/(?:regions|zones)/([^/?]+)/([^/?]+)/([^/?]+)', group)
      if not m:
        raise RuntimeError(f"Can't determine region or zone or group name from self links {group}")
      ...
      if resource_type == 'instanceGroups':
        ...
        if network_endpoint_group:
          filter_str = """resource.type="gce_network_endpoint_group"
                        ...
                        jsonPayload.healthCheckProbeResult.healthState="UNHEALTHY"
                        """.format(self.project_id, network_endpoint_group.id, location)
        else:
          op.add_skipped(
            resource=self.backend_service,
            reason=(
        ...
        op.add_skipped(
          resource=self.backend_service,
          reason=(
            f'Unsupported resource type {resource_type} for group'
      ...
      if serial_log_entries:
        ...
        if get_path(last_log, 'jsonPayload.healthCheckProbeResult.healthState') == 'UNHEALTHY':
...
class AnalyzeTimeoutHealthCheckLog(runbook.Step):
  ...
  def name(self):
    return (
      f'Analyze TIMEOUT health check logs for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    probe_results_texts = {get_path(log, 'probeResultText') for log in self.logs}
...
class AnalyzeUnhealthyHealthCheckLog(runbook.Step):
  ...
  def name(self):
    return (
      f'Analyze UNHEALTHY health check logs for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    op.add_uncertain(
      ...
      remediation=op.prep_msg(op.FAILURE_REMEDIATION, success_criteria=success_criteria),
    )
...
class AnalyzeUnknownHealthCheckLog(runbook.Step):
  ...
  def name(self):
    return (
      f'Analyze UNKNOWN health check logs for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    op.add_uncertain(
      self.backend_service,
      reason=op.prep_msg(op.FAILURE_REASON, bs_url=self.backend_service.full_path),
      remediation=op.prep_msg(op.FAILURE_REMEDIATION),
    )
...
class CheckPastHealthCheckSuccess(runbook.Step):
  ...
  def name(self):
    return (
      f'Check past health check success for backend service'
      f' "{self.backend_service_name}" in scope'
      f' "{self.region}".'
    )
  ...
  def execute(self):
    ...
    for group in unhealthy_groups:
      m = re.search(r'/(?:regions|zones)/([^/?]+)/([^/?]+)/([^/?]+)', group)
      if not m:
        raise RuntimeError(f"Can't determine region or zone or instanceGroup name from ig {group}")
      ...
      if serial_log_entries:
        ...
        message += f'{group}: Backends transitioned to an unhealthy state at {timestamp}\n\n'
      else:
        message += f'{group}: No logs were found indicating HEALTHY -> UNHEALTHY transition \n\n'

    if message and any_date_found:
      op.add_uncertain(
        self.backend_service,
        reason=message,
        remediation=op.prep_msg(op.UNCERTAIN_REMEDIATION, bs_url=self.backend_service.full_path),
      )
    else:
      op.add_skipped(
        self.backend_service,
        reason=(
          'No past health check success found in the logs for the '
...
def get_health_check_success_criteria(health_check: gce.HealthCheck):
  ...
  success_criteria = f'The health check is using {health_check.type} protocol, and '

  port = (
    'serving port'
    if health_check.port_specification == 'USE_SERVING_PORT'
    else f'port {health_check.port}'
  )
  ...
  if health_check.type in ['HTTP', 'HTTPS', 'HTTP2']:
    ...
    if health_check.response:
      success_criteria += f' and response body containing the string "{health_check.response}"'

  elif health_check.type in ['TCP', 'SSL']:
    success_criteria += f'it is set to: \n- send a prober requests on {port}'
    if health_check.request:
      success_criteria += f' with the configured request string "{health_check.request}"'
    success_criteria += f'\n- expect a successful {health_check.type} handshake'
    if health_check.response:
      success_criteria += f' and the response string exactly matches: "{health_check.response}"'
...
) -> Optional[gce.NetworkEndpointGroup]:
