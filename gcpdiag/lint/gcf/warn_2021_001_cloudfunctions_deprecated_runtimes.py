...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  for cloudfunction in sorted(
    cloudfunctions.values(), key=lambda cloudfunction: cloudfunction.name
  ):
    if cloudfunction.runtime in DEPRECATED_RUNTIMES:
      report.add_failed(
        cloudfunction, f'cloudfunction with deprecated runtime {cloudfunction.runtime}'
      )
    else:
