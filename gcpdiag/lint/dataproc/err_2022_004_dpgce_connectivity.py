  ...
  for cluster in clusters:
    ...
    if cluster.status != 'ERROR':
      cluster_details.append(
        {'cluster': cluster, 'skipped': True, 'reason': 'Cluster not in error state'}
      )
      continue
    ...
    if not cluster.is_gce_cluster:
      cluster_details.append(
        {'cluster': cluster, 'skipped': True, 'reason': 'Dataproc on GKE cluster'}
      )
      continue

    # Skip single node clusters
    if cluster.is_single_node_cluster:
      cluster_details.append({'cluster': cluster, 'skipped': True, 'reason': 'Single node cluster'})
      continue

    # target
    target = gce.get_instance(
      project_id=context.project_id, zone=cluster.zone, instance_name=f'{cluster.name}-m'
    )
    nic = None
    ...
    if nic:
      effective_firewalls = gce.get_instance_interface_effective_firewalls(target, nic)
    else:
      # log error
      continue
    ...
    source = gce.get_instance(
      project_id=context.project_id, zone=cluster.zone, instance_name=f'{cluster.name}-w-1'
    )

    cluster_details.append(
      {
        'cluster': cluster,
        'firewalls': effective_firewalls,
        'source_ip': source.get_network_ip_for_instance_interface(cluster.gce_network_uri),
        'source_sa': source.service_account,
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface) -> None:
  for detail in cluster_details:
    ...
    if icmp.action == 'deny':
      report.add_failed(
        cluster, f'ICMP connections must be allowed, blocked by: {icmp.matched_by_str}'
      )
      continue
    ...
    tcp = firewalls.check_connectivity_ingress(
      src_ip=source_ip,
      ip_protocol='TCP',
      port=port,
      source_service_account=source_sa,
    ...
    if tcp.action == 'deny':
      report.add_failed(
        cluster, f'TCP connections must be allowed on all ports, blocked by: {tcp.matched_by_str}'
      )
      continue

    # check random udp port
    udp = firewalls.check_connectivity_ingress(
      src_ip=source_ip,
      ip_protocol='UDP',
      port=port,
      source_service_account=source_sa,
    ...
    if udp.action == 'deny':
      report.add_failed(
        cluster, f'UDP connections must be allowed on all ports, blocked by: {udp.matched_by_str}'
      )
      continue
