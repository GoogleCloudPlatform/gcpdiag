...
class FakeModulesGateway:
  ...
  def list_pkg_modules(self, pkg):
    return [k for k in self.modules_by_name.keys() if k.startswith(pkg.__name__)]
...
class Setup:
  ...
  def __init__(self, modules_by_name, load_extended=None, include=None, exclude=None):
    self.modules_by_name = modules_by_name
  ...
  @cached_property
  def repo(self):
    return LintRuleRepository(
      load_extended=self.load_extended,
...
def test_sync_happy_path():
  ...
  executed_run_rule_fs = [r.run_rule_f for r in setup.execution_strategy.executed_rules]
...
def test_async_happy_path():
  ...
  assert len(setup.execution_strategy.executed_rules) == 2
  executed_run_rule_fs = [r.async_run_rule_f for r in setup.execution_strategy.executed_rules]
...
def test_empty_doc_raises():
  fake_module1 = FakeModule(methods={'run_rule': lambda context, rule_report: None}, doc='')
...
def test_multiline_doc_happy_path():
  ...
  assert len(setup.execution_strategy.executed_rules) == 1
  assert setup.execution_strategy.executed_rules[0].short_desc == 'first line '
  assert setup.execution_strategy.executed_rules[0].long_desc == ' third line \n fourth line'
