      ...
      for query in logs_by_project.values():
        for log_entry in query.entries:
          try:
            ...
            logging.warning(
              'Skip: Failed to get cluster name/location from log entry %s', log_entry
            )
            continue

          if log_cluster_location == c.location and log_cluster_name == c.name:
            pod_name = re.search(r'pod=\"(.*?\/.*?)-.*?\"', log_entry['jsonPayload']['MESSAGE'])
            if pod_name:
              pod_names.add(pod_name.group(1))
      report.add_failed(c, reason='Failing pods in namespace/workload: ' + ', '.join(pod_names))
    else:
      report.add_ok(c)
