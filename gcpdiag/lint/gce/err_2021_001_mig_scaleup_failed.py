...
def prepare_rule(context: models.Context):
  migs = [m for m in gce.get_managed_instance_groups(context).values() if not m.is_gke()]
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  migs: List[gce.ManagedInstanceGroup] = [
    mi for mi in gce.get_managed_instance_groups(context).values() if not mi.is_gke()
  ]
  if not migs:
    ...
    for log_entry in query.entries:
      try:
        ...
        mig_list = list(
          filter(
            lambda x: x.is_instance_member(
              log_entry['resource']['labels']['project_id'], region, instance_name
            ),
            migs,
          )
        )
        ...
        if log_entry['protoPayload']['status']['message'] == 'LIMIT_EXCEEDED':
          ...
          errors_by_mig[mig_list[0]] = log_entry['protoPayload']['status']['message']
      except KeyError:
        continue
