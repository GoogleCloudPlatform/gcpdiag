...
def prepare_rule(context: models.Context):
  vm_oom_pattern = ['Out of memory: Kill process', 'Kill process', 'Memory cgroup out of memory']
  ...
  vm_disk_space_error_pattern = [
    ...
    'A stop job is running for Security ...',
  ...
  global mem_search
  mem_search = utils.SerialOutputSearch(
    context, search_strings=vm_oom_pattern, custom_filter=filter_oom_str
  )

  global disk_search
  disk_search = utils.SerialOutputSearch(
    context, search_strings=vm_disk_space_error_pattern, custom_filter=filter_disk_str
  )

  # Fetching the list of instances in the project
  global instances
  instances = [vm for vm in gce.get_instances(context).values() if not vm.is_gke_node()]
    ...
    if not i.is_running:
        ...
        if disk.type in ['pd-balanced', 'pd-ssd', 'pd-standard', 'pd-extreme']:
          disk_io_latency = monitoring.query(
            ...
            """
              ...
              | {}
            """.format(i.id, disk.name, disk.type, IO_LATENCY_THRESHOLD, within_str),
          )
          ...
          if disk_io_latency:
            ...
            report.add_ok(i, ('Disk {} IO Latency is under optimal levels').format(disk.name))
        else:
          report.add_skipped(
            i,
            reason=(
      ...
      if cpu_usage:
        report.add_failed(
          i,
          (
            'CPU utilization is exceeding optimal levels, potentially '
            'impacting performance of the instance.'
          ),
        )
      else:
      ...
      if mem_usage:
        ...
        report.add_failed(
          i,
          (
            'Found OOM related events in Serial console logs, potentially '
            'impacting performance of the instance.'
          ),
        )
      else:
      ...
      if disk_usage:
        ...
        report.add_failed(
          i,
          (
            'Found Disk related errors/events in Serial console logs, '
