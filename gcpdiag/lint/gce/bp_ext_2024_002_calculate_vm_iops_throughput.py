...
def prepare_rule(context: models.Context):
  global instances
  instances = [vms for vms in gce.get_instances(context).values() if not vms.is_gke_node()]
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  for vm in sorted(instances):
    if not vm.is_running:
      ...
      within_hours = 9
      if start_dt_utc_plus_10_mins > current_time_utc and (isinstance(vm.laststoptimestamp(), str)):
      ...
      try:
        ...
        report.add_skipped(
          resource=None, reason=('Not able to pull CPU count for instance {}').format(vm)
        )
      ...
      if cpu_count_query:
        ...
        report.add_skipped(
          ...
          short_info=('\n\tPlease first start the VM {}, if it is not in running state').format(
            vm.short_path
          ),
        )
        return
      ...
      for disk_name in disk_list.items():
        ...
        if disk.type == 'pd-balanced':
          ...
          report.add_skipped(
            vm,
            reason=(
              'Disk-Type {} is not supported with this gcpdiag runbook, disk name - {}'
            ).format(disk.type, disk.name),
          )
