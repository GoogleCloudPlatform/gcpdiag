...
def find_logs_with_permission_errors(
  context: models.Context, logs_by_project: dict, principals_with_error: set
):
  if logs_by_project.get(context.project_id) and logs_by_project[context.project_id].entries:
    for log_entry in logs_by_project[context.project_id].entries:
      ...
      if (
        ...
        and MATCH_STRING in get_path(log_entry, ('protoPayload', 'status', 'message'), default='')
        and principal_email not in principals_with_error
      ):
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  principals_with_error.update(
    find_logs_with_permission_errors(context, logs_by_project_compute, principals_with_error)
  )
