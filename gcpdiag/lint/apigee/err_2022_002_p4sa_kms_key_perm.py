...
def _run_rule_kms_key(
  ...
  if kms_key_name:
    ...
    if _is_valid_kms_key(kms_key):
      if not _apigee_sa_has_role_permissions(context, kms_key_name, service_account):
        report.add_failed(
          kms_key,
          (
      ...
      report.add_failed(kms_key, f'Key {kms_key_name} is not valid (destroyed or disabled)')
  else:
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  _run_rule_kms_key(
    context, report, kms_key_name, RUNTIME_DATABASE_ENCRYPTION_KEY_STRING, service_account
  )
  ...
  for instance in sorted(instances_list.values(), key=lambda instance: instance.name):
    ...
    _run_rule_kms_key(context, report, kms_key_name, DISK_ENCRYPTION_KEY_STRING, service_account)
...
def _apigee_sa_has_role_permissions(context: models.Context, kms_key_name, service_account):
  iam_policy = kms.get_crypto_key_iam_policy(context, kms_key_name)
  return iam_policy.has_role_permissions(f'serviceAccount:{service_account}', ROLE)
