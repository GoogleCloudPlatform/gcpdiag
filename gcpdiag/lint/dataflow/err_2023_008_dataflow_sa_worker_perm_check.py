...
LOG_FILTER = [
  ...
  ('protoPayload.methodName="dataflow.jobs.updateContents" OR "dataflow.jobs.create"'),
]
logs_by_project = {}
policies_by_project = {}
...
def run_rule(context: models.Context, report: lint.LintReportRuleInterface):
  ...
  if context.project_id in logs_by_project:
    ...
    for log_entry in logs_by_project[context.project_id].entries:
      ...
      sa_dataflow_worker_role = policies_by_project[context.project_id].has_role_permissions(
        member=f'serviceAccount:{service_account}', role=WORKER_ROLE
      )

      sa_owner_role = policies_by_project[context.project_id].has_role_permissions(
        member=f'serviceAccount:{service_account}', role=OWNER_ROLE
      )

      sa_editor_role = policies_by_project[context.project_id].has_role_permissions(
