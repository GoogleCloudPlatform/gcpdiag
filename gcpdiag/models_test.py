...
def test_context_to_string():
  ...
  assert str(c) == (
    'project: project1, locations (regions/zones): us-central1, parameters: {project_id=project1}'
  )

  c = models.Context(
    project_id='project1', locations=['us-west1', 'us-west2'], resources=['dev-1', 'prod-1']
  )
  assert str(c) == (
  ...
  assert str(c) == 'project: project1, labels: {A=B,X=Y}, parameters: {project_id=project1}'

  c = models.Context(
    project_id='project1', locations=['us-central1'], labels={'X': 'Y'}, resources=['name']
  )
  assert str(c) == (
...
def test_match_project_resource():
  ...
  assert c.match_project_resource(resource='gke-prod', location='us-central1', labels={'X': 'Y'})
  assert not c.match_project_resource(resource='', location='us-central1', labels={'X': 'Y'})
  ...
  assert c.match_project_resource(
    resource='bastion-host', location='us-central1', labels={'X': 'Y'}
  )
  assert c.match_project_resource(
    resource='dev-frontend', location='us-central1', labels={'X': 'Y'}
  )
  assert c.match_project_resource(resource='dev-backend', location='us-central1', labels={'X': 'Y'})
  assert not c.match_project_resource(resource='', location='us-central1', labels={'X': 'Y'})
  assert not c.match_project_resource(resource='bastion-host', location='', labels={'X': 'Y'})
  assert not c.match_project_resource(
    resource='bastion-host', location='us-central1', labels={'X': 'B'}
  )
  assert not c.match_project_resource(resource='name', labels={'X': 'Y'}, location='us-central3')
  assert not c.match_project_resource(
  ...
  assert c.match_project_resource(resource='bastion-host', location='us-central2-a')

  # Test IGNORELOCATION AND IGNORELABEL
  assert c.match_project_resource(
    resource='bastion-host', location=c.IGNORELOCATION, labels=c.IGNORELABEL
  )

  # If for some strange coincidence customer's label is IGNORELABEL evaluation
  # should fail if it doesn't match context.
  assert not c.match_project_resource(
    resource='bastion-host', labels={'IGNORELABEL': 'IGNORELABEL'}
  )
