...
def fetch_and_list_steps(output_file_path: str, base_url_path: str):
    ...
    for file in files:
      if file.endswith('.py') and not file.startswith('__'):
        ...
        modules_with_files.append(
          (module_path, file_url)
        )  # gcpdiag.runbook.nameofrunbook.nameoffile
  ...
  for module_path, file_url in modules_with_files:
    try:
      ...
      continue
    for _, obj in inspect.getmembers(module, inspect.isclass):  # returns Only class
      if issubclass(obj, runbook.Step) and obj is not runbook.Step:
        try:
          step_instance = obj()  # obj of the Class
          if hasattr(step_instance, 'type') and isinstance(step_instance.type, constants.StepType):
  ...
  for row in rows_data:
    ...
    table_rows.append(
      '|[{file_name}]({file_url}) | **{step_name}** | {step_type} | `{step_id}` |'.format(
        **row_values
      )
    )
...
def main(argv: Sequence[str]) -> None:
  ...
  output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'steps_table.md')
